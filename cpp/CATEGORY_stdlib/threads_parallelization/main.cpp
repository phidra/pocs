...
int main(void)
{
  ...
  const size_t nb_of_threads = 4;  // see also  std::thread::hardware_concurrency();
  {
    ...
    for(int thread_index = 0; thread_index < nb_of_threads; thread_index++)
    {
        ...
        const int loop_end_for_this_thread = (thread_index+1) == nb_of_threads ? nloop : (thread_index+1) * nloop / nb_of_threads;
        auto worker_with_args = std::bind(
            worker_function,
            loop_begin_for_this_thread,
            loop_end_for_this_thread
        );
