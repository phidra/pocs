        ...
        for (unsigned i = 0; i < query_count; ++i) {
            try {
                ...
                if (ch_query.get_distance() != ref_distance[i])
                    throw runtime_error("Distance query " + std::to_string(i) +
                                        " is wrong; reference = " + std::to_string(ref_distance[i]) +
                                        " computed = " + std::to_string(ch_query.get_distance()));
                ...
                if (source[i] == target[i]) {
                    ...
                    if (arc_path.empty()) {
                        ...
                        if (distance != ref_distance[i])
                            throw runtime_error(
                                "path has wrong length; reference = " + std::to_string(ref_distance[i]) +
                                " computed = " + std::to_string(distance));
                        ...
                        for (unsigned j = 0; j < arc_path.size(); ++j)
                            if (head[arc_path[j]] != node_path[j + 1])
                                throw runtime_error(
                                    "get_node_path path has a different node than induced by the arc path.");
                    }
                }
