    ...
    try {
        ...
        while (cin >> source >> target) {
            ...
            cout << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                    ...
                    "\t\t<name>Source</name>\n"
                    "\t\t<Point><coordinates>"
                 << std::setprecision(10) << longitude[source] << ',' << std::setprecision(10) << latitude[source]
                 << ",0</coordinates></Point>\n"
                    "\t</Placemark>\n"
                    "\t<Placemark>\n"
                    "\t\t<name>Target</name>\n"
                    "\t\t<Point><coordinates>"
                 << std::setprecision(10) << longitude[target] << ',' << std::setprecision(10) << latitude[target]
                 << ",0</coordinates></Point>\n"
                    "\t</Placemark>\n"
                    "\t<Placemark>\n"
                    "\t\t<name>Path of length "
                 << ch_query.get_distance()
                 << " meter</name>\n"
                    "\t\t<LineString>\n"
                    "\t\t\t<coordinates>\n";
            for (unsigned x : ch_query.get_node_path())
                cout << "\t\t\t\t" << std::setprecision(10) << longitude[x] << ',' << std::setprecision(10)
                     << latitude[x] << ",0\n";
            cout << "\t\t\t</coordinates>\n"
                    "\t\t</LineString>\n"
                    "\t</Placemark>\n"
