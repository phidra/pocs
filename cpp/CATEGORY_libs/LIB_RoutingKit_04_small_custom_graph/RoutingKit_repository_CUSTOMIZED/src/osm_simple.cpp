...
SimpleOSMCarRoutingGraph simple_load_osm_car_routing_graph_from_pbf(
    ...
    auto mapping = load_osm_id_mapping_from_pbf(
        pbf_file,
        nullptr,
        [&](uint64_t osm_way_id, TagMap const& tags) { return is_osm_way_used_by_cars(osm_way_id, tags, log_message); },
        log_message,
    ...
    auto routing_graph = load_osm_routing_graph_from_pbf(
        ...
        [&](uint64_t osm_relation_id,
            ...
            return decode_osm_car_turn_restrictions(
                osm_relation_id, member_list, tags, on_new_restriction, log_message);
        },
        log_message);
