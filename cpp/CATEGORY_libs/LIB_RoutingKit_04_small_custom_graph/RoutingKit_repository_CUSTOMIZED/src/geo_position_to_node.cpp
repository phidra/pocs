        ...
        // Faster more complex vectorized code
        {
            ...
            for (unsigned i = mid; i < end; i += vector_width) {
                ...
                for (unsigned j = 0; j < vector_width; ++j) {
                    ...
                    a_[j] = sinf(dlat * 0.5) * sinf(dlat * 0.5) +
                            sinf(dlon * 0.5) * sinf(dlon * 0.5) * cosf(a_lat) * cosf(b_lat[j]);
                }

                for (unsigned j = 0; j < element_count; ++j) {
                    // GCC5 does not have vectorized versions of atan2f nor sqrtf
                    // we therefore call the sequential functions as they would prevent the vectorization of the
                    // previous loop
...
void nearest_neighbor_recursion(std::vector<GeoPositionToNode::PointPosition> const& point_position,
    ...
    if (end - begin <= max_points_per_leaf) {
        ...
        //				assert(compute_distance(pivot_position, point_position[i]) <=
        // pivot_boundary_distance); 			for(unsigned i=mid+1; i<end; ++i)
...
GeoPositionToNode::NearestNeighborhoodQueryResult GeoPositionToNode::find_nearest_neighbor_within_radius(
