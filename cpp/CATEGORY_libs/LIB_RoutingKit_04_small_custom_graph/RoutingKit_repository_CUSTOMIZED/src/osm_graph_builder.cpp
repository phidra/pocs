...
OSMRoutingIDMapping load_osm_id_mapping_from_pbf(std::string const& file_name,
    ...
    if (log_message) {
        ...
        log_message("OSM ID range goes up to " + std::to_string(map.is_modelling_node.size()) +
                    " for modelling nodes.");
...
OSMRoutingGraph load_osm_routing_graph_from_pbf(
    ...
    auto on_new_arc = [&](unsigned x,
        ...
        if (geometry_to_be_extracted == OSMRoadGeometry::uncompressed) {
            ...
            routing_graph.modelling_node_latitude.insert(routing_graph.modelling_node_latitude.end(),
                                                         modelling_node_latitude.begin(),
                                                         modelling_node_latitude.end());
            routing_graph.modelling_node_longitude.insert(routing_graph.modelling_node_longitude.end(),
                                                          modelling_node_longitude.begin(),
...

    {
        ...
        routing_graph.is_arc_antiparallel_to_way =
            apply_inverse_permutation(p, std::move(routing_graph.is_arc_antiparallel_to_way));
        ...
        if (geometry_to_be_extracted == OSMRoadGeometry::uncompressed ||
            ...
            for (auto old_arc_id : new_arc_id_to_old_arc_id) {
                ...
                modelling_node_latitude.insert(modelling_node_latitude.end(),
                                               routing_graph.modelling_node_latitude.begin() + first,
                                               routing_graph.modelling_node_latitude.begin() + last);
                modelling_node_longitude.insert(modelling_node_longitude.end(),
    ...
    if (!osm_turn_restrictions.empty()) {
...

        {
            ...
            auto in = osm_turn_restrictions.begin(), out = osm_turn_restrictions.begin(),
        ...
        if (log_message) {
            log_message("After removing restrictions with not exported ways or node only " +
                        std::to_string(osm_turn_restrictions.size()) + " restrictions remain.");
        }
...

        {
...

            {
                if (log_message) {
                    log_message(
                        "Start filling in missing via node IDs in turn restrictions and handling mandatory turns with "
                        "from = to");
                    timer = -get_micro_time();
                }
                ...
                while (in != end) {
                    if (in->via_node != invalid_id) {
                        *out = *in;
                        ...
                        for (auto i = first_index_of_way[in->from_way]; i != first_index_of_way[in->from_way + 1];
                             ++i) {
                        ...
                        for (auto i = first_index_of_way[in->from_way]; i != first_index_of_way[in->from_way + 1];
                            ...
                            if (in_arc != invalid_id)
                                for (unsigned not_out_arc = first_out[via_node]; not_out_arc != first_out[via_node + 1];
                                     ++not_out_arc)
                        ...
                        if (via_node == invalid_id) {
                            if (log_message)
                                log_message("Turn restriction with OSM-relation-ID \"" +
                                            std::to_string(in->osm_relation_id) +
                                            "\" "
                                            "with OSM-way-from-ID \"" +
                                            std::to_string(routing_way.to_global(in->from_way)) +
                                            "\" "
                                            "and OSM-way to-ID \"" +
                                            std::to_string(routing_way.to_global(in->to_way)) +
                                            "\" "
                            ...
                            if (log_message)
                                log_message("Turn restriction with OSM-relation-ID \"" +
                                            ...
                                            "\" "
                                            "does not have a via-node "
                                            "and there are multiple ambiguous candidates, namely OSM-node-IDs \"" +
                                            std::to_string(routing_node.to_global(via_node)) +
                                            "\" "
                                            "and \"" +
                                            std::to_string(routing_node.to_global(second_via_node)) +
                                            "\" (and maybe more), ingoring restriction");
                            ++multiple_via_count;
                            ++in;
                        } else {
                            ++repaired_count;
                ...
                if (log_message) {
                    log_message("There were " + std::to_string(go_straight_count) +
                                " go-straight turns that were expanded into " + std::to_string(forbidden_from.size()) +
                                " forbidden turns.");
                    log_message("There were " + std::to_string(repaired_count) +
                                " normal turns without via-node for which a via-node could be derived.");
                    log_message("There were " + std::to_string(no_via_count) +
                                " normal turns without via-node discarded because no potential via-node was found.");
                    log_message(
                        "There were " + std::to_string(no_via_count) +
                        " normal turns without via-node discarded because multiple potential via-node were found.");
                }
...

        {
            ...
            auto in_arc =
                compute_sort_permutation_using_key(routing_graph.head, node_count, [](unsigned x) { return x; });
            ...
            for (auto x : osm_turn_restrictions) {
                ...
                if (from_candidates.empty()) {
                    // if(log_message){
                    // 	log_message(
                    // 		"Cannot find from-arc for turn restriction with OSM relation ID
                    // \""+std::to_string(x.osm_relation_id)+"\" " 		"and OSM from-way
                    // \""+std::to_string(routing_way.to_global(x.from_way))+"\" " 		"and OSM to-way
                    // \""+std::to_string(routing_way.to_global(x.to_way))+"\" " 		"and OSM via-node
                    // \""+std::to_string(routing_node.to_global(x.via_node))+"\""
                ...
                for (unsigned arc = routing_graph.first_out[x.via_node]; arc != routing_graph.first_out[x.via_node + 1];
                     ++arc) {
                    assert(arc < arc_count);
                ...
                if (to_candidates.empty()) {
                    // if(log_message){
                    // 	log_message(
                    // 		"Cannot find to-arc for turn restriction with OSM relation ID
                    // \""+std::to_string(x.osm_relation_id)+"\" " 		"and OSM from-way
                    // \""+std::to_string(routing_way.to_global(x.from_way))+"\" " 		"and OSM to-way
                    // \""+std::to_string(routing_way.to_global(x.to_way))+"\" " 		"and OSM via-node
                    // \""+std::to_string(routing_node.to_global(x.via_node))+"\""
                ...
                if (from_candidates.size() == 1 && to_candidates.size() == 1) {
                    ...
                    for (unsigned from_cand : from_candidates) {
                        ...
                        if (routing_graph.first_modelling_node[from_cand] ==
                            routing_graph.first_modelling_node[from_cand + 1]) {
                            from_lat = routing_graph.latitude[tail[from_cand]];
                            from_lon = routing_graph.longitude[tail[from_cand]];
                        } else {
                            from_lat =
                                routing_graph
                                    .modelling_node_latitude[routing_graph.first_modelling_node[from_cand + 1] - 1];
                            from_lon =
                                routing_graph
                                    .modelling_node_longitude[routing_graph.first_modelling_node[from_cand + 1] - 1];
                        }
                        ...
                        for (unsigned to_cand : to_candidates) {
                            ...
                            if (routing_graph.first_modelling_node[to_cand] ==
                                routing_graph.first_modelling_node[to_cand + 1]) {
                                to_lat = routing_graph.latitude[routing_graph.head[to_cand]];
                                to_lon = routing_graph.longitude[routing_graph.head[to_cand]];
                            } else {
                                to_lat =
                                    routing_graph.modelling_node_latitude[routing_graph.first_modelling_node[to_cand]];
                                to_lon =
                                    routing_graph.modelling_node_longitude[routing_graph.first_modelling_node[to_cand]];
                            }
                    ...
                    if (matching_candidate_count >= 2) {
                        log_message("OSM turn restriction relation ID \"" + std::to_string(x.osm_relation_id) +
                                    "\" "
                                    "is a turn restriction where it is impossible to infer the restriction without "
                                    "using the turn direction information. However, " +
                                    std::to_string(matching_candidate_count) +
                                    " restriction candidates are consistent "
...

        {
            ...
            auto p = compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
                arc_count, forbidden_from, forbidden_to);
            forbidden_to = apply_inverse_permutation(p, move(forbidden_to));

            assert(is_sorted_using_less(forbidden_from));

            BitVector is_duplicate = make_bit_vector(forbidden_from.size(), [&](unsigned x) {
                if (x == 0)
                    return false;
                else
