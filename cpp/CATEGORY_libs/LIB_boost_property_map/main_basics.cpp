...
//
// Quel intérêt ?
//      certains algos doivent fetcher/setter les valeurs d'un dictionnaire (e.g. dijkstra, pour affecter un
//      tentative_cost à un node) grâce à property_map, ces algos n'ont pas à dépendre d'un type concret (std::map), et
//      ne dépendent que d'une interface (la property_map). cette interface peut être implémentée par plusieurs types,
//      p.ex. std::map, un std::vector, etc.
//
...
// Chacune a une doc spécifique, listées dans la page de doc (p.ex. :
// https://www.boost.org/doc/libs/1_74_0/libs/property_map/doc/associative_property_map.html)
//      identity    : valeur = clé
...
//      R = avec ma compréhension du problème, on peut pas, il faut utiliser la "vraie" structure (ici, std::map)
//      directement. Q = comment savoir si une clé est valide ? R = avec ma compréhension du problème, on peut pas, il
//      faut utiliser la "vraie" structure (ici, std::map) directement.

// Le point intéressant : foo ne dépend pas de std::map, mais uniquement de "quelque chose implémentant
// put/get/operator[]" Justement, property map est l'interface représentant ce "quelque chose implémentant
// put/get/operator[] (ici, à partir d'une std::map)
...
int main() {
    ...
    // Après appel de foo, on constate que les adresses dans la map (name2address) ont été modifiées (alors que foo a
    // travaillé sur la property_map) :
