...
void usage(string prog_name) {
    ...
    cerr << "\t LINE 4 = analysed node's ORIGINAL successor nodes (same field as above, each group being separated by "
            "'|')"
         << endl;
    cerr << "\t LINE 5 = analysed node's CONTRACTED successor nodes (there are additional fields containing the nodes "
            "of the unpacked shortcut, as rkindex)"
         << endl;
    cerr << endl;
...
int main(int argc, char* argv[]) {
    ...
    cerr << "The ANALYZED NODE is the one with rank : " << RANK << " (which has rkindex " << NODE << ") : " << OSMURL
         << endl;
    cerr << endl;
    ...
    for (unsigned arc = ch.forward.first_out[RANK]; arc < ch.forward.first_out[RANK + 1]; ++arc) {
        ...
        cerr << "\t [" << arc << "] " << RANK << ">" << head << " (weight=" << weight << "), with URL : " << head_osmurl
             << endl;
