...
void convert_int_data(string const& output_vector_file) {
    ...
    while (getline(cin, line)) {
        ...
        if (x < numeric_limits<T>::min())
            throw runtime_error("The number \"" + line + "\" is too small, min is \"" +
                                to_string(numeric_limits<T>::min()) + "\"");
        if (x > numeric_limits<T>::max())
            throw runtime_error("The number \"" + line + "\" is too large, max is \"" +
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc != 3) {
            cerr << "Usage: " << argv[0] << " data_type output_vector_file\n"
                 << "\n"
                 << "Reads textual data from the standard input and writes it in a binary format to "
                    "output_vector_file. The input data should be one data element per line. The data is only written "
                    "once an end of file is encountered on the input. data_type can be one of\n"
                 << " * int8\n"
                 << " * uint8\n"
                 << " * int16\n"
                 << " * uint16\n"
