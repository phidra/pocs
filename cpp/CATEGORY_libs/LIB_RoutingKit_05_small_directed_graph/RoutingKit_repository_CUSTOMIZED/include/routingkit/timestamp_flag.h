...
class TimestampFlags {
   public:
    ...
    bool is_set(unsigned id) const { return last_seen[id] == current_timestamp; }

    void set(unsigned id) { last_seen[id] = current_timestamp; }

    void reset_one(unsigned id) { last_seen[id] = current_timestamp - 1; }
