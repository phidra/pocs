...
bool seek_impl(F&& func, T&& value, sv json_ptr) {
    ...
    if constexpr (glaze_array_t<std::decay_t<T>>) {
        ...
        if (index >= std::tuple_size_v<std::decay_t<T>>)
            return false;
...
template <class R>
using call_return_t = std::conditional_t<std::is_reference_v<R>,
                                         expected<std::reference_wrapper<std::decay_t<R>>, error_code>,
                    ...
                    case '{': {
                        ...
                        while (true) {
                            ...
                            if (cx_string_cmp<key>(k)) {
                                ...
                                if (*it != ',') {
                                    ret = unexpected(parse_error{error_code::key_not_found,
