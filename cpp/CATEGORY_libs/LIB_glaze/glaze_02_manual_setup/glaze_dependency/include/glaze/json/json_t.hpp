...
struct json_t {
    ...
    template <class T>
    requires std::convertible_to<T, val_t> &&(!std::same_as<json_t, std::decay_t<T>>)json_t(T&& val) { data = val; }

    template <class T>
    requires std::convertible_to<T, double> &&
        (!std::same_as<json_t, std::decay_t<T>>)&&(!std::convertible_to<T, val_t>)json_t(T&& val) {
