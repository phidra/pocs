...
GLZ_ALWAYS_INLINE unsigned char hex2dec(char hex) {
                ...
                while (it < end) {
                    if constexpr (!Opts.force_conformance) {
                        ...
                        switch (*it) {
                            ...
                            [[unlikely]] case '\b' : [[unlikely]] case '\f' : [[unlikely]] case '\n'
                                : [[unlikely]] case '\r' : [[unlikely]] case '\t' : {
                                ctx.error = error_code::syntax_error;
                                return;
                            }
                            [[unlikely]] case '\0' : {
...
template <class T>
requires(str_view_t<T> || char_array_t<T>) struct from_json<T> {
    template <auto Opts, class It, class End>
    GLZ_ALWAYS_INLINE static void op(auto& value, is_context auto&& ctx, It&& it, End&& end) noexcept {
        if constexpr (!Opts.opening_handled) {
            if constexpr (!Opts.ws_handled) {
...
template <class T>
requires(std::is_enum_v<T> && !glaze_enum_t<T>) struct from_json<T> {
    template <auto Opts>
    GLZ_ALWAYS_INLINE static void op(auto& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept {
...
// for set types
template <class T>
requires(readable_array_t<T> && !emplace_backable<T> && !resizeable<T> && emplaceable<T>) struct from_json<T> {
...
template <auto Opts>
[[nodiscard]] GLZ_ALWAYS_INLINE size_t number_of_array_elements(is_context auto&& ctx, auto it, auto&& end) noexcept {
    skip_ws_no_pre_check<Opts>(ctx, it, end);
    if (bool(ctx.error)) [[unlikely]]
        return {};
...
template <class T>
requires readable_array_t<T> &&(!emplace_backable<T> && resizeable<T>)struct from_json<T> {
    template <auto Options>
    GLZ_FLATTEN static void op(auto& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept {
        if constexpr (!Options.ws_handled) {
            skip_ws<Options>(ctx, it, end);
...
requires(Opts.error_on_unknown_keys) GLZ_ALWAYS_INLINE std::string_view
    parse_object_key(is_context auto&& ctx, auto&& it, auto&& end) {
                                            ...
                                            if (id_it != id_map.end()) [[likely]] {
                                                ...
                                                std::visit(
                                                    [&](auto&& v) {
                                                        ...
                                                        constexpr bool is_object = glaze_object_t<V>;
                                                        if constexpr (is_object) {
                                                            from_json<V>::template op<opening_handled<Opts>(),
