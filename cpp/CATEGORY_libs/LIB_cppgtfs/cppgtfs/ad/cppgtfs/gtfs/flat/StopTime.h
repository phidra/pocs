...
struct Time {
    ...
    std::string toString() const {
        std::stringstream ss;
        if (!empty()) {
            ss << std::setfill('0') << std::setw(2) << static_cast<int>(h) << ":" << std::setfill('0') << std::setw(2)
               << static_cast<int>(m) << ":" << std::setfill('0') << std::setw(2) << static_cast<int>(s);
        }
        return ss.str();
    }
    uint8_t m : 6;
...
struct StopTime {
    enum PU_DO_TYPE : uint8_t { REGULAR = 0, NEVER = 1, MUST_PHONE_AGENCY = 2, MUST_COORDINATE_W_DRIVER = 3 };
