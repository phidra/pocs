...
bool CsvParser::readNextLine() {
    ...
    if (readN > 2 && static_cast<int>(_buff[0]) == -17 && static_cast<int>(_buff[1]) == -69 &&
        static_cast<int>(_buff[2]) == -65) {
        pos = 3;
        lastPos = pos;
    }
...
int32_t CsvParser::getLong(const size_t i) const {
    if (i >= _currentItems.size())
        throw CsvParserException("expected non-negative integer number", i, getFieldName(i), _curLine);
    bool fail = false;
