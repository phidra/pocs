...
class ServiceDate {
   public:
    ...
    uint8_t getMonth() const { return (_yyyymmdd - ((getYear() - 1900) * 10000)) / 100; }
    uint8_t getDay() const { return _yyyymmdd - ((getYear() - 1900) * 10000) - (getMonth() * 100); }
    ...
    void setYear(uint16_t year) { _yyyymmdd = _yyyymmdd - (getYear() - 1900) * 10000 + (year - 1900) * 10000; }
...
struct CalendarDate {
    enum EXCEPTION_TYPE : uint8_t { NOT_SET = 0, SERVICE_ADDED = 1, SERVICE_REMOVED = 2 };
