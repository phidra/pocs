...
//
// Ce qui va se passer :
//      0. le conteneur étant un std::array, on n'est théoriquement pas capable de retrouver un âge autrement qu'à partir d'un size_t (l'index de l'âge)
//      1. mais là intervient la property-map, et son OffsetMap (ce qu'en dit la doc : the value type must be convertible to the difference type of the iterator)
//      2. étant donné une clé (nom d'une personne), on lit l'OffsetMap, pour récupérer l'offset où aller lire l'âge dans le tableau, pour la personne concernée
//         d'une certaine façon, ce comportement ressemble aux hashmaps.
//         En effet, dans une hashmap, la clé est quelconque, mais les éléments sont stockés dans un tableau indexé par des entiers.
//         Il faut donc convertir la clé en un entier (l'index), pour une hashmap, c'est la fonction de hachage qui fait cette conversion.
//         Dans l'iterator property-map, on n'a pas de fonction de hachage, mais on a une table de conversion : l'OffsetMap
//
...
//
// Je pense que l'intérêt, c'est quand on dispose DÉJÀ d'un tableau stockant des objets (e.g. un graphe stocké en tant qu'adjacency_list avec un vector)
// Dans ce cas, la property-map est un Adapter, présentant une interface de type dictionnaire au dessus du vector pré-existant "contenant" le graphe.
// Et l'intérêt des PropertyMap pour la BGL, c'est qu'on dispose déjà des données (le vector des vertex du graphe), et de l'offset-map (le vertex_index du graphe)

struct Age { int age ; Age(int x) : age{x} {} };

std::ostream& operator<<(std::ostream& out, Age const& x) { out << x.age; return out; }
...
int main(void)
{
    ...
    // Construction de la property-map, à partir du tableau (contenant les properties), et de la map associant une clé à un indes dans le tableau :
    auto pmap = boost::make_iterator_property_map(my_array.begin(), my_offset_map_that_works);

    // À partir de maintenant, pmap me permet d'avoir une interface de type dictionnaire (get/put utilisant des clés de type string) sur un std::array :-)
