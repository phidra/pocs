...
Graph build_graph(map<WayId, vector<LocatedNode> > const& way_to_nodes, map<NodeOsmId, int> const& number_of_node_usage) {
    ...
    for (auto ite : way_to_nodes) {
        ...
        while (first_node != last_node) {
            ...
            if (second_node == nodes.end()) {
                add_edge(boost_graph, osmid2descriptor, first_node->first, (second_node - 1)->first, std::move(geometry));
                break;
            }

            // cas général : on ajoute l'edge, et on continue d'iéterer sur la way :
            geometry.push_back(second_node->second);
            add_edge(boost_graph, osmid2descriptor, first_node->first, second_node->first, std::move(geometry));
...
Graph osm_to_graph(string osmfile, BgPolygon polygon) {
    // osmium warmup :
    ...
    // build graph edges :
    auto boost_graph = build_graph(handler.way_to_nodes, handler.node_use_counter);
