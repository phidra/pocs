...
int main(int, char*[]) {
    ...
    cout << "   - la façon de définir des properties custom pour les vertex (resp. edges), de deux façons : ancienne "
            "(boost::property) ou nouvelle (bundled properties)"
         << endl;
    cout << "   - la façon d'associer PLUSIEURS properties aux vertex (resp. edges), y compris des properties custom "
            "(dont les tags n'existent pas dans BGL)"
         << endl;
    cout << "   - deux façons de créer un vertex (resp. edge) avec ses properties : en créant la property "
            "préalablement, ou à la volée via un initializer-list"
         << endl;
    cout << "   - la façon de lire les properties d'un vertex (resp. edge), avec l'ANCIENNE comme la NOUVELLE façon de "
    ...
    // type-tag (je ne peux pas utiliser la même structure, car l'un doit avoir kind=edge_property_tag, et l'autre doit
    // avoir vertex) :
    struct tomtom_vertex_id_t {
    ...
    // NOUVELLE FAÇON DE FAIRE = bundled properties (cf.
    ...
    auto tentative_distances_property_map =
        make_iterator_property_map(tentative_distances.begin(), get(vertex_index, mygraph));
    ...
    cout << "Le plus court chemin de " << nameof(SOURCE) << " vers " << nameof(TARGET)
         << " a pour poids total : " << get(tentative_distances_property_map, TARGET) << endl;
    ...
    // QUESTION 1 = si j'ai une paire de vertex (source+target), comment puis-je savoir si l'edge source->target existe,
    // et en récupérer un descriptor ? a priori, on peut le faire avec adjacency_matrix, mais pas avec adjacency_list
