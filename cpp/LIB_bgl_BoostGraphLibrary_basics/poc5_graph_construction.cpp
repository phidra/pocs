...
int main(int,char*[])
{
    ...
    cout << "   - l'utilisateur du même constructor que dans la POC 3 (permettant de construire le graphe à partir de ses edges)" << endl;
    cout << "   - le fait de setter a posteriori les properties des vertex (en les récupérant à partir de leurs indices)" << endl;
    ...
    // Il permet de construire un graphe avec ses edges (munis de leur edge-properties), des vertices (éventuellement vides).
    // En revanche, on dirait qu'il ne permet pas de définir les properties des vertex.
    // TODO : un contournement que j'envisage (à tester dans une POC future) serait de définir les VertexProperties comme external-properties ?
    // (c'est pas fi-fou, notamment car les vertex-properties ont vocation à être internes)
    //
    ...
    vector<EdgeProperty> the_properties = {
        {5, 999001, "AB"},
        {10, 999002, "AC"},
        {1, 999003, "AD"},
        {8, 999004, "BC"},
    ...
    // Pour récupérer un VertexDescriptor à partir de leur indice, je suppose (apparemment à raison) que les vertices ont été insérés dans l'ordre de définition plus haut.
    // (une alternative aurait été de récupérer les 8 vertex en 8 appels du genre "source(this_specific_edge, mygraph)", mais c'était moins pratique)

    // Cette lambda me permet de setter les propriétés d'un vertex à partir de son indice (indice utilisé dans le vector the_edges plus haut) :
    ...
    // check que les edges et les vertex sont bien cohérents (la concaténation du name de source+target donne le name de l'edge) :
