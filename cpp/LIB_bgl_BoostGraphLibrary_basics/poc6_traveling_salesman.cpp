...
int main(int, char*[]) {
    ...
    cout << "   - l'utilisation de back_inserter pour créer un OutputIterator requis par l'algo (la doc n'était pas "
            "très explicite...)"
         << endl;
    cout << endl;
    ...
    // (ça laisse tout de même 2520 chemins possibles, cf.
    ...
    for (int city_index = 0; city_index != cities.size(); ++city_index) {
        // ceci suppose que les vertex ont été créé dans le même ordre que celui par lequel ils sont référencés dans
        // the_edges :
    ...
    //      back_inserter is a convenient function template that constructs a std::back_insert_iterator for the
    //      container c
    // https://en.cppreference.com/w/cpp/iterator/back_insert_iterator
    //      The container's push_back() member function is called whenever the iterator (whether dereferenced or not) is
    //      assigned to.

    metric_tsp_approx_tour_from_vertex(mygraph, SOURCE, weight_map, result_iterator);
