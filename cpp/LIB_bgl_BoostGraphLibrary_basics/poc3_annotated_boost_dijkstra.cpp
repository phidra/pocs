...
int main(int, char*[]) {
    std::cout << std::endl;
    std::cout << "Cette POC n'est qu'une version annotée de l'exemple donné avec boost — mais c'est déjà pas mal !"
              << std::endl;
    ...
    // comme rien n'est précisé explicitement, la directness du graphe a la valeur par défaut pour adjacency_list =
    // directed
    //
    typedef graph_traits<Graph>::vertex_descriptor vertex_descriptor;

    // ========================================
    // STEP 2 = toute cette STEP ne sert qu'à appeler le constructeur du graphe pour l'initialiser avec les bons
    ...
    // appel au 6ième constructor de la section "Member functions" de cette page :
    ...
    // ceci est simplement la récupération de la property_map des weights, permettant, à partir d'un edge donné, de
    // récupérer son weight :
    ...
    dijkstra_shortest_paths(graph, SOURCE,
                            ...
                            predecessor_map(parents_property_map)
                                .  // ceci est une ReadWritePropertyMap stockant pour chaque vertex V, son prédécesseur
                                   // sur le plus court chemin reliant SOURCE à V
                            distance_map(distances_property_map)  // ceci est une ReadWritePropertyMap stockant pour
                                                                  // chaque vertex V sa tentative-distance
    );

    // ========================================
    // STEP 6 = on affiche les distances et les parents (permettant de reconstituer si besoin les plus courts chemins) :
    // rien de sorcier ici, hormis le fait que c'est parce qu'on a utilisé boost::vertex_index comme OffsetMap qu'on
    // peut utiliser la syntaxe bien pratique :
    ...
    for (boost::tie(vi, vend) = vertices(graph); vi != vend; ++vi) {
        ...
        std::cout << "parent(" << name[*vi] << ") = " << name[parents[*vi]] << std::endl;
    }
    std::cout << std::endl;
    ...
    //      - on on récupère le poids d'un edge, en utilisant l'interface property_map (de la weightmap) alors même que
    //      c'était une internal-property :)
    // le seul point qui m'interpelle un peu dans ce code, c'est qu'on assimile allégrement le vertex_descriptor à son
    // index dans l'enum (alors que je m'attendrais à ce que ce lien soit au mieux fait la fonction "vertex" , au pire
    // ne puisse pas être fait directement)
    graph_traits<Graph>::edge_iterator ei, ei_end;
    for (boost::tie(ei, ei_end) = edges(graph); ei != ei_end; ++ei) {
        graph_traits<Graph>::edge_descriptor e = *ei;
        graph_traits<Graph>::vertex_descriptor u = source(e, graph),
                                               v = target(e, graph);  // ici, u et v sont des vertex_descriptor
