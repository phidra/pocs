...
std::vector<unsigned> compute_inverse_sort_permutation_first_by_left_then_by_right(unsigned a_count,
    ...
    auto q = compute_inverse_stable_sort_permutation_using_key(
        apply_inverse_permutation(p, std::move(a)), a_count, [](unsigned x) { return x; });
...
std::vector<unsigned> compute_sort_permutation_first_by_left_then_by_right(unsigned a_count,
    ...
    auto q = compute_stable_sort_permutation_using_key(
        apply_permutation(p, std::move(a)), a_count, [](unsigned x) { return x; });
    assert(is_sorted_using_less(a));
...
std::vector<unsigned> compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
    ...
    std::vector<unsigned> const& head) {
    return compute_inverse_sort_permutation_first_by_left_then_by_right_and_apply_sort_to_left(
        node_count, tail, node_count, head);
}
...
std::vector<unsigned> compute_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
    ...
    std::vector<unsigned>& tail,
    std::vector<unsigned> const& head) {
    return compute_sort_permutation_first_by_left_then_by_right_and_apply_sort_to_left(
        node_count, tail, node_count, head);
}
