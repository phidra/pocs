...
unsigned find_arc_or_return_invalid(const std::vector<unsigned>& first_out,
...
unsigned find_arc_given_sorted_head(const std::vector<unsigned>& first_out,
                                    const std::vector<unsigned>& head,
                                    unsigned x,
                                    unsigned y) {
...
unsigned find_arc_or_return_invalid_given_sorted_head(const std::vector<unsigned>& first_out,
                                                      const std::vector<unsigned>& head,
                                                      unsigned x,
                                                      unsigned y) {
    assert(x < first_out.size() - 1 && "node id out of bounds");
    ...
    auto begin = head.begin() + first_out[x], end = head.begin() + first_out[x + 1];
...
std::vector<unsigned> convert_node_path_to_arc_path(const std::vector<unsigned>& first_out,
                                                    const std::vector<unsigned>& head,
...
std::vector<unsigned> compute_inverse_sort_permutation_first_by_left_then_by_right(unsigned a_count,
    ...
    auto q = compute_inverse_stable_sort_permutation_using_key(apply_inverse_permutation(p, std::move(a)), a_count,
                                                               [](unsigned x) { return x; });
...
std::vector<unsigned> compute_sort_permutation_first_by_left_then_by_right(unsigned a_count,
    ...
    auto q = compute_stable_sort_permutation_using_key(apply_permutation(p, std::move(a)), a_count,
                                                       [](unsigned x) { return x; });
    assert(is_sorted_using_less(a));
...
std::vector<unsigned> compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
    ...
    const std::vector<unsigned>& head) {
    return compute_inverse_sort_permutation_first_by_left_then_by_right_and_apply_sort_to_left(node_count, tail,
                                                                                               node_count, head);
}
...
std::vector<unsigned> compute_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
    ...
    std::vector<unsigned>& tail,
    const std::vector<unsigned>& head) {
    return compute_sort_permutation_first_by_left_then_by_right_and_apply_sort_to_left(node_count, tail, node_count,
                                                                                       head);
}
