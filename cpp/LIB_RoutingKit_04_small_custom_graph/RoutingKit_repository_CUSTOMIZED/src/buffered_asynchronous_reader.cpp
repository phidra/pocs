...
BufferedAsynchronousReader::BufferedAsynchronousReader(
    ...
    impl->worker = std::thread([this, ptr, byte_source, block_size] {
        std::unique_lock<std::mutex> guard(ptr->lock);
        try {
            for (;;) {
                ptr->main_thread_has_done_something.wait(guard, [&] {
                    return (blocks_per_buffer * block_size - ptr->how_many_bytes_are_in_the_buffer()) >=
                               2 * block_size ||
                           ptr->was_termination_requested;
                });
...
char* BufferedAsynchronousReader::read(unsigned size) {
    if (size > impl->block_size)
        throw std::runtime_error("Requested to read " + std::to_string(size) +
                                 " bytes, which is more than the block size of " + std::to_string(impl->block_size));

    std::unique_lock<std::mutex> guard(impl->lock);

    impl->worker_thread_has_done_something.wait(guard, [&] {
        return impl->was_end_of_file_reached || impl->how_many_bytes_are_in_the_buffer() >= size ||
               impl->read_exception;
    });
...
char* BufferedAsynchronousReader::read_or_throw(unsigned size) {
    char* x = read(size);
    if (x == 0)
        throw std::runtime_error("Wanted to read " + std::to_string(size) + " bytes but only " +
                                 std::to_string(impl->how_many_bytes_are_in_the_buffer()) +
                                 " are available in the data source.");
    return x;
}
