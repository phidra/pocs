...
FileDataSource::FileDataSource(FileDataSource&& o) : file_descriptor(o.file_descriptor) {
    o.file_descriptor = -1;
}
...
void FileDataSource::open(char const* file_name) {
    ...
    if (new_file_descriptor == -1) {
        int error = errno;
        throw std::runtime_error(std::string("Could not open file \"") + file_name + "\" for reading. The errno is " +
                                 std::to_string(error) + ". strerror(errno) says the following : " + strerror(error));
    }
...
void FileDataSource::rewind() {
    if (::lseek(file_descriptor, 0, SEEK_SET) == -1) {
        int error = errno;
        throw std::runtime_error(std::string("Could not rewind a file open for reading. The errno is ") +
                                 std::to_string(error) + ". strerror(errno) says the following : " + strerror(error));
    }
}
    ...
    if (file_descriptor == -1)
        ...
        if (!::fstat(file_descriptor, &buf)) {
            ...
            throw std::runtime_error(
                std::string("Could not determine the size of a file open for reading. The errno is ") +
                std::to_string(error) + ". strerror(errno) says the following : " + strerror(error));
        }
        return buf.st_size;
    }
...
unsigned long long my_read(int file_descriptor, char* buffer, unsigned long long to_read) {
    ...
    if (s < 0) {
        int error = errno;
        throw std::runtime_error(std::string("Could not read from file. The errno is ") + std::to_string(error) +
                                 ". strerror(errno) says the following : " + strerror(error));
    }
    return s;
}
}  // namespace
...
unsigned long long FileDataSource::size() const {
    if (file_descriptor == nullptr)
        ...
        if (fseek(file_descriptor, x, SEEK_SET))
            throw std::runtime_error(
                "fseek failed on file opened for reading, file stream could be in an unexpected state.");

        if (x == -1L)
            throw std::runtime_error(
                "ftell failed on file opened for reading, file stream could be in an unexpected state.");
...
std::function<unsigned long long(char*, unsigned long long)> FileDataSource::get_read_function_object() {
    auto x =
        file_descriptor;  // C++ or GCC 4.8 (I do not know who is at fault) does not like capturing members by value...
    return [x](char* buffer, unsigned long long to_read) -> unsigned long long { return my_read(x, buffer, to_read); };
}
