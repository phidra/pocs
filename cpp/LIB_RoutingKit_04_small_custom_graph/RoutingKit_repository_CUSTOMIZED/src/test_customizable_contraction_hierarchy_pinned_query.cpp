    ...
    try {
...

        {
            ...
            for (unsigned s = 0; s < source_count; ++s) {
                for (unsigned t = 0; t < target_count; ++t) {
                    optimal_result[s * target_count + t] =
                        query.reset().add_source(source_set[s]).add_target(target_set[t]).run().get_distance();
                }
            }
...

        {
            ...
            for (unsigned s = 0; s < source_count; ++s) {
                ...
                auto d =
                    query.reset_source().add_source(source_set[s]).run_to_pinned_targets().get_distances_to_targets();
            ...
            cout << "query_time"
                 << " : " << query_time / query_count << "musec" << endl;
            cout << "pin_time"
                 << " : " << select_time / select_count << "musec" << endl;
        }
...

        {
            ...
            for (unsigned t = 0; t < target_count; ++t) {
                query_time -= get_micro_time();
                auto d =
                    query.reset_target().add_target(target_set[t]).run_to_pinned_sources().get_distances_to_sources();
                for (unsigned s = 0; s < source_count; ++s) {
