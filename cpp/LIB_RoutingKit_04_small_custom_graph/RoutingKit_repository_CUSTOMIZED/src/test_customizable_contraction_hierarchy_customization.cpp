...
int main(int argc, char* argv[]) {
    try {
        ...
        if (reference_metric.forward != parallel_metric.forward ||
            reference_metric.backward != parallel_metric.backward) {
        ...
        if (reference_metric.forward != partial_metric.forward ||
            reference_metric.backward != partial_metric.backward) {
        ...
        for (unsigned a = 0; a < reference_metric.forward.size(); ++a) {
            if (reference_metric.forward[a] != partial_metric.forward[a])
                throw std::runtime_error("Partial Customization is broken: forward weight of " + std::to_string(a) +
                                         " is " + std::to_string(partial_metric.forward[a]) + " but should be " +
                                         std::to_string(reference_metric.forward[a]));
            if (reference_metric.backward[a] != partial_metric.backward[a])
                throw std::runtime_error("Partial Customization is broken: backward weight of " + std::to_string(a) +
                                         " is " + std::to_string(partial_metric.forward[a]) + " but should be " +
                                         std::to_string(reference_metric.forward[a]));
        }
