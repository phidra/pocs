...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        ...
        cout << "Usage: " << argv[0] << " test.pbf [test_count]\n"
             << "Example: \n"
...

    {
        ContractionHierarchyQuery q(ch);
        {
            ...
            for (unsigned i = 0; i < test_count; ++i) {
                ...
                q.reset().add_source(source1[i]).add_target(target1[i]).run();
                ch_one_to_one_query_time += get_micro_time();
            ...
            log_message("CH one-to-one distance query running time average over " + to_string(test_count) +
                        " uniform random queries : " + to_string(ch_one_to_one_query_time / test_count) + "musec");
            for (unsigned i = 0; i < test_count; ++i) {
                q.reset()
...

    {
        ...
        CustomizableContractionHierarchyQuery q(m);
        {
            ...
            log_message("CCH one-to-one distance query running time average over " + to_string(test_count) +
                        " uniform random queries : " + to_string(cch_one_to_one_query_time / test_count) + "musec");
            for (unsigned i = 0; i < test_count; ++i) {
                q.reset()
                    .add_source(source1[i])
                    .add_source(source2[i])
...

    {
        ...
        ContractionHierarchyQuery q(perfect_ch);
        {
            ...
            log_message("Perfectly customized CCH one-to-one distance query running time average over " +
                        to_string(test_count) + " uniform random queries : " +
                        to_string(cch_perfect_one_to_one_query_time / test_count) + "musec");
            for (unsigned i = 0; i < test_count; ++i) {
                q.reset()
                    .add_source(source1[i])
                    .add_source(source2[i])
...

    {
        ...
        {
            for (unsigned i = 0; i < test_count; ++i) {
                q.reset().add_source(source1[i], source1_offset[i]).add_target(target1[i], target1_offset[i]).run();
                shortest_path_distance1[i] = q.get_distance();
...

    {
        ...
        ch_query.reset().pin_targets(target1);
        ch_pin_time += get_micro_time();

        long long cch_pin_time = -get_micro_time();
        cch_query.reset().pin_targets(target1);
        cch_pin_time += get_micro_time();

        log_message("CH pin targets running time with " + to_string(target1.size()) +
                    " targets : " + to_string(ch_pin_time) + "musec");
        log_message("CCH pin targets running time with " + to_string(target1.size()) +
                    " targets : " + to_string(ch_pin_time) + "musec");

        long long ch_one_to_many_time = 0, cch_one_to_many_time = 0, ch_naive_one_to_many = 0,
                  cch_naive_one_to_many = 0;
        for (unsigned i = 0; i < test_count; ++i) {
            ch_one_to_many_time -= get_micro_time();
            ch_query.reset_source().add_source(source1[i], source1_offset[i]).run_to_pinned_targets();
            ch_one_to_many_time += get_micro_time();
            cch_one_to_many_time -= get_micro_time();
            cch_query.reset_source().add_source(source1[i], source1_offset[i]).run_to_pinned_targets();
            cch_one_to_many_time += get_micro_time();

            auto d = ch_query.get_distances_to_targets();
            for (unsigned j = 0; j < test_count; ++j) {
                ch_naive_one_to_many -= get_micro_time();
                EXPECT_CMP(ref_query.reset()
                               .add_source(source1[i], source1_offset[i])
        ...
        log_message("Naive CH pinned 1-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(ch_naive_one_to_many / test_count) + "musec");
        log_message("Naive CCH pinned 1-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(cch_naive_one_to_many / test_count) + "musec");
        log_message("CH pinned 1-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(ch_one_to_many_time / test_count) + "musec");
        log_message("CCH pinned 1-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(cch_one_to_many_time / test_count) + "musec");
    }

    {
        ...
        log_message("Naive CH pinned 3-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(ch_naive_one_to_many / test_count) + "musec");
        log_message("CH 3-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(ch_one_to_many_time / test_count) + "musec");
        log_message("CCH 3-to-target-query running time averaged over " + to_string(test_count) +
                    " runs : " + to_string(cch_one_to_many_time / test_count) + "musec");
    }

    {
        ...
        ch_query.reset().pin_sources(source1);
        cch_query.reset().pin_sources(source1);
        for (unsigned i = 0; i < test_count; ++i) {
