...
LocalIDMapper::LocalIDMapper(uint64_t bit_count, uint64_t const* bits) :
    ...
    for (uint64_t j = 0; j < rank_.size() - 1; ++j) {
        rank_[j] = s;
        uint64_t s0 = __builtin_popcountll(bits_[i + 0]), s1 = __builtin_popcountll(bits_[i + 1]),
                 s2 = __builtin_popcountll(bits_[i + 2]), s3 = __builtin_popcountll(bits_[i + 3]),
                 s4 = __builtin_popcountll(bits_[i + 4]), s5 = __builtin_popcountll(bits_[i + 5]),
                 s6 = __builtin_popcountll(bits_[i + 6]), s7 = __builtin_popcountll(bits_[i + 7]);
