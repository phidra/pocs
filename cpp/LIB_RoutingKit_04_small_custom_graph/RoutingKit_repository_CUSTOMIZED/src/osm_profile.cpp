...
bool is_osm_way_used_by_pedestrians(uint64_t osm_way_id,
    ...
    if (str_eq(highway, "secondary") || str_eq(highway, "tertiary") || str_eq(highway, "unclassified") ||
        str_eq(highway, "residential") || str_eq(highway, "service") || str_eq(highway, "secondary_link") ||
        str_eq(highway, "tertiary_link") || str_eq(highway, "living_street") || str_eq(highway, "residential") ||
        str_eq(highway, "track") || str_eq(highway, "bicycle_road") || str_eq(highway, "path") ||
        str_eq(highway, "footway") || str_eq(highway, "cycleway") || str_eq(highway, "bridleway") ||
        str_eq(highway, "pedestrian") || str_eq(highway, "escape") || str_eq(highway, "steps") ||
        str_eq(highway, "crossing") || str_eq(highway, "escalator") || str_eq(highway, "elevator") ||
        str_eq(highway, "platform") || str_eq(highway, "ferry"))
        return true;

    if (str_eq(highway, "motorway") || str_eq(highway, "motorway_link") || str_eq(highway, "motorway_junction") ||
        str_eq(highway, "trunk") || str_eq(highway, "trunk_link") || str_eq(highway, "primary") ||
        str_eq(highway, "primary_link") || str_eq(highway, "construction") || str_eq(highway, "bus_guideway") ||
        str_eq(highway, "raceway") || str_eq(highway, "proposed") || str_eq(highway, "conveying"))
        return false;
...
bool is_osm_way_used_by_cars(uint64_t osm_way_id,
    ...
    if (str_eq(highway, "motorway") || str_eq(highway, "trunk") || str_eq(highway, "primary") ||
        str_eq(highway, "secondary") || str_eq(highway, "tertiary") || str_eq(highway, "unclassified") ||
        str_eq(highway, "residential") || str_eq(highway, "service") || str_eq(highway, "motorway_link") ||
        str_eq(highway, "trunk_link") || str_eq(highway, "primary_link") || str_eq(highway, "secondary_link") ||
        str_eq(highway, "tertiary_link") || str_eq(highway, "motorway_junction") || str_eq(highway, "living_street") ||
        str_eq(highway, "residential") || str_eq(highway, "track") || str_eq(highway, "ferry"))
        return true;
    ...
    if (str_eq(highway, "construction") || str_eq(highway, "path") || str_eq(highway, "footway") ||
        str_eq(highway, "cycleway") || str_eq(highway, "bridleway") || str_eq(highway, "pedestrian") ||
        str_eq(highway, "bus_guideway") || str_eq(highway, "raceway") || str_eq(highway, "escape") ||
...
OSMWayDirectionCategory get_osm_car_direction_category(uint64_t osm_way_id,
    ...
    if (oneway != nullptr) {
        if (str_eq(oneway, "-1") || str_eq(oneway, "reverse") || str_eq(oneway, "backward")) {
            ...
            if (log_message)
                log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has unknown oneway tag value \"" +
                            oneway + "\" for \"oneway\". Way is closed.");
        }
...
unsigned parse_maxspeed_value(uint64_t osm_way_id,
    ...
    if (str_eq(maxspeed, "de:rural") || str_eq(maxspeed, "at:rural") || str_eq(maxspeed, "ro:rural") ||
    ...
    if ('0' <= *maxspeed && *maxspeed <= '9') {
        ...
        if (*maxspeed == '\0' || str_eq(maxspeed, "km/h") || str_eq(maxspeed, "kmh") || str_eq(maxspeed, "kph")) {
            ...
            if (log_message)
                log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has an unknown unit \"" + maxspeed +
                            "\" for its \"maxspeed\" tag -> assuming \"km/h\".");
            return speed;
        }
    } else {
        if (log_message)
            log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has an unrecognized value of \"" +
                        maxspeed + "\" for its \"maxspeed\" tag.");
    }
...
unsigned get_osm_way_speed(uint64_t osm_way_id,
    ...
    if (maxspeed != nullptr && !str_eq(maxspeed, "unposted")) {
        ...
        if (speed == 0) {
            speed = 1;
            if (log_message)
                log_message("Warning: OSM way " + std::to_string(osm_way_id) +
                            " has speed 0 km/h, setting it to 1 km/h");
        }
    ...
    if (maxspeed && highway && log_message)
        log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has an unrecognized \"maxspeed\" tag of \"" +
                    maxspeed + "\" and an unrecognized \"highway\" tag of \"" + highway +
                    "\" and an no junction tag -> assuming 50km/h.");
    if (!maxspeed && highway && log_message)
        log_message("Warning: OSM way " + std::to_string(osm_way_id) +
                    " has no \"maxspeed\" and an unrecognized \"highway\" tag of \"" + highway +
                    "\" and an no junction tag -> assuming 50km/h.");
    if (!maxspeed && !highway && log_message)
        log_message("Warning: OSM way " + std::to_string(osm_way_id) +
                    " has no \"maxspeed\" and no \"highway\" tag and an no junction tag -> assuming 50km/h.");
    if (maxspeed && !highway && log_message)
        log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has an unrecognized \"maxspeed\" tag of \"" +
                    maxspeed + "\" and no \"highway\" tag and an no junction tag -> assuming 50km/h.");
    return 50;
}
...
bool is_osm_way_used_by_bicycles(uint64_t osm_way_id,
    ...
    if (str_eq(highway, "secondary") || str_eq(highway, "tertiary") || str_eq(highway, "unclassified") ||
        str_eq(highway, "residential") || str_eq(highway, "service") || str_eq(highway, "secondary_link") ||
        str_eq(highway, "tertiary_link") || str_eq(highway, "living_street") || str_eq(highway, "residential") ||
        str_eq(highway, "track") || str_eq(highway, "bicycle_road") || str_eq(highway, "primary") ||
        str_eq(highway, "primary_link") || str_eq(highway, "path") || str_eq(highway, "footway") ||
        str_eq(highway, "cycleway") || str_eq(highway, "bridleway") || str_eq(highway, "pedestrian") ||
        str_eq(highway, "crossing") || str_eq(highway, "escape") || str_eq(highway, "steps") ||
        str_eq(highway, "ferry"))
        return true;

    if (str_eq(highway, "motorway") || str_eq(highway, "motorway_link") || str_eq(highway, "motorway_junction") ||
        str_eq(highway, "trunk") || str_eq(highway, "trunk_link") || str_eq(highway, "construction") ||
        str_eq(highway, "bus_guideway") || str_eq(highway, "raceway") || str_eq(highway, "conveying"))
        return false;
...
OSMWayDirectionCategory get_osm_bicycle_direction_category(uint64_t osm_way_id,
    ...
    if (oneway_bicycle != nullptr) {
        ...
        if (str_eq(oneway_bicycle, "1") || str_eq(oneway_bicycle, "yes") || str_eq(oneway_bicycle, "true") ||
            str_eq(oneway_bicycle, "no_planned"))
            return OSMWayDirectionCategory::only_open_forwards;

        if (str_eq(oneway_bicycle, "0") || str_eq(oneway_bicycle, "no") || str_eq(oneway_bicycle, "false") ||
            str_eq(oneway_bicycle, "tolerated") || str_eq(oneway_bicycle, "permissive"))
            return OSMWayDirectionCategory::open_in_both;
        if (log_message)
            log_message("Warning: OSM way " + std::to_string(osm_way_id) + " has unknown oneway tag value \"" +
                        oneway_bicycle + "\" for \"oneway:bicycle\". Way is closed.");
        return OSMWayDirectionCategory::closed;
    }
    ...
    if (oneway == nullptr) {
        ...
        if (cycleway != nullptr) {
            ...
            //
            // A consequence is that "cycleway=opposite" combined "oneway=-1" does not imply that bicycles are only
            // allowed to drive backwards
            //
            // (Yes, people actually do combine those two tags https://www.openstreetmap.org/way/88925376 )
            if (str_eq(cycleway, "opposite") || str_eq(cycleway, "opposite_track") ||
                str_eq(cycleway, "opposite_lane") || str_eq(cycleway, "opposite_share_busway")) {
                return OSMWayDirectionCategory::open_in_both;
            }
        }
...
void decode_osm_car_turn_restrictions(uint64_t osm_relation_id,
    ...
    if (starts_with("only_", restriction)) {
        ...
        if (log_message)
            log_message("Unknown OSM turn restriction with ID " + std::to_string(osm_relation_id) + " and value \"" +
                        restriction + "\", ignoring restriction");
        return;
    }
    ...
    for (unsigned i = 0; i < member_list.size(); ++i) {
        if (str_eq(member_list[i].role, "via")) {
            if (via_member != invalid_id) {
                if (log_message)
                    log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                                " has several \"via\" roles, ignoring restriction");
                return;
            }
            via_member = i;
            ...
            if (log_message)
                log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) + " and unknown role \"" +
                            member_list[i].role + "\", ignoring role");
        }
    }
    ...
    if (via_member != invalid_id && member_list[via_member].type == OSMIDType::relation) {
        if (log_message)
            log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                        " has a relation as \"via\"-role, this is invalid, ignoring restriction");
        return;
    }

    if (via_member != invalid_id && member_list[via_member].type == OSMIDType::way) {
        // log_message("OSM turn restriction with ID "+std::to_string(osm_relation_id)+" and name \""+restriction+"\"
        // has a relation as \"way\"-role, this feature is not supported, ignoring restriction");
        return;
    }
        ...
        std::remove_if(from_member_list.begin(), from_member_list.end(),
                       [&](unsigned member) {
                           if (member_list[member].type != OSMIDType::way) {
                               if (log_message)
                                   log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                                               " has \"from\"-role that is not a way, ignoring role");
                               return true;
                           } else {
                               return false;
                           }
    ...
    to_member_list.erase(std::remove_if(to_member_list.begin(), to_member_list.end(),
                                        [&](unsigned member) {
                                            if (member_list[member].type != OSMIDType::way) {
                                                if (log_message)
                                                    log_message("OSM turn restriction with ID " +
                                                                std::to_string(osm_relation_id) +
                                                                " has \"to\"-role that is not a way, ignoring role");
                                                return true;
                                            } else {
                                                return false;
                                            }
    ...
    if (to_member_list.empty()) {
        if (log_message)
            log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                        " is missing \"to\" role, ignoring restriction");
        return;
    }
    ...
    if (from_member_list.empty()) {
        if (log_message)
            log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                        " is missing \"from\" role, ignoring restriction");
        return;
    }
    ...
    if (restriction_type == OSMTurnRestrictionCategory::mandatory && to_member_list.size() != 1) {
        if (log_message)
            log_message("OSM turn restriction with ID " + std::to_string(osm_relation_id) +
                        " is mandatory but has several \"to\" roles, ignoring restriction");
        return;
    }
