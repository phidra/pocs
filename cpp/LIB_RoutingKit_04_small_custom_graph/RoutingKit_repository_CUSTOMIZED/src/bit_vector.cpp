...
#ifdef _MSC_VER
#include <malloc.h>
// Let us hope that this workaround does not break when MS finally implements the C11 standard function aligned_alloc
// from <stdlib.h>... If it does break then please open an issue with the information of upto what values of _MSC_VER
...
// Not all compilers support aligned_alloc such as GCC 4.6. If you have such a compiler then define
// ROUTING_KIT_NO_ALIGNED_ALLOC
...
BitVector::BitVector() : data_(nullptr), size_(0) {}
...
BitVector::BitVector(uint64_t size, bool init_value) {
    if (__builtin_expect(size != 0, true)) {
        ...
        for (v8_uint64_t* i = (v8_uint64_t*)data_; i < ((v8_uint64_t*)data_) + get_v8_uint64_count(size); ++i)
            *i = init_vec;
...
BitVector::BitVector(BitVector&& o) : data_(o.data_), size_(o.size_) {
    assert(are_all_padding_bits_zero(o));
