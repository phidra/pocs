...
class TagMap {
   private:
    static unsigned compute_char_hash(char c) {
        switch (c) {
            ...
            case 'R':
                return 15;
            default:
                return static_cast<unsigned char>(c) % 7;
        }
    }
    ...
    static unsigned compute_hash(const char* str) {
        ...
        return ((compute_char_hash(str[0])) * char_hash_count + compute_char_hash(str[last_char])) * char_hash_count +
   ...
   public:
    ...
    bool empty() const { return entry.empty(); }
    ...
    std::vector<Entry>::const_iterator end() const { return entry.end(); }
