...
std::tuple<unsigned, unsigned, unsigned, bool> get_edge_info(unsigned xy,
...
void _out_edges_of(unsigned x_rank, RoutingKit::ContractionHierarchy const& ch, TNameOf const& nameof, Direction dir) {
    ...
    for (unsigned xy = graph.first_out[x_rank]; xy < graph.first_out[x_rank + 1]; ++xy) {
        ...
        std::cout << "\t[id=" << xy << "]  " << dir_string << original_string << nameof(x_node)
                  << nameof(edge_head_node) << " has weight " << edge_weight << std::endl;
        if (!is_edge_original) {
            ...
            std::cout << "\t\tJe m'intéresse au PREMIER edge de ce shortcut, qui a pour id : " << first_edge_id
                      << std::endl;
            ...
            std::cout << "\t\t\tCe first edge est-il original     ? " << std::boolalpha
                      << ch.forward.is_shortcut_an_original_arc.is_set(first_edge_id) << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_first_arc  F? " << ch.forward.shortcut_first_arc[first_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_second_arc F? " << ch.forward.shortcut_second_arc[first_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_first_arc  B? " << ch.backward.shortcut_first_arc[first_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_second_arc B? " << ch.backward.shortcut_second_arc[first_edge_id]
                      << std::endl;
            std::cout << "\t\t\tMIDDLE NODE PUTATIF ??????????????? "
                      << nameof(ch.backward.shortcut_second_arc[first_edge_id]) << std::endl;

            std::cout << "\t\tJe m'intéresse au SECOND edge de ce shortcut, qui a pour id : " << second_edge_id
                      << std::endl;
            ...
            std::cout << "\t\t\tCe second edge est-il original    ? " << std::boolalpha
                      << ch.forward.is_shortcut_an_original_arc.is_set(second_edge_id) << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_first_arc  F? " << ch.forward.shortcut_first_arc[second_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_second_arc F? " << ch.forward.shortcut_second_arc[second_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_first_arc  B? " << ch.backward.shortcut_first_arc[second_edge_id]
                      << std::endl;
            std::cout << "\t\t\tQuel est son shortcut_second_arc B? " << ch.backward.shortcut_second_arc[second_edge_id]
                      << std::endl;
            std::cout << "\t\t\tMIDDLE NODE PUTATIF ??????????????? "
                      << nameof(ch.backward.shortcut_second_arc[second_edge_id]) << std::endl;
            ...
            /* std::cout << "TRAITEMENT PARTICULIER : middle_node name = " << nameof(ch.order[middle_node_rank]) <<
             * std::endl; */
            std::cout << std::endl;

            auto [first_edge_head_rank, first_edge_head_node, first_edge_weight, is_first_edge_original] =
                get_edge_info(first_edge_id, ch, BACKWARD);
            auto [second_edge_head_rank, second_edge_head_node, second_edge_weight, is_second_edge_original] =
                get_edge_info(second_edge_id, ch, FORWARD);
            std::cout << "\t\t FIRST  edge of the shortcut = " << nameof(x_node) << nameof(first_edge_head_node)
                      << "  (weight=" << first_edge_weight << ", "
                      << (is_first_edge_original ? "ORIGINAL)" : "SHORTCUT)") << std::endl;
            std::cout << "\t\t SECOND edge of the shortcut = " << nameof(second_edge_head_node) << nameof(xy_head_node)
                      << "  (weight=" << second_edge_weight << ", "
                      << (is_second_edge_original ? "ORIGINAL)" : "SHORTCUT)") << std::endl;
        }
    }
}
