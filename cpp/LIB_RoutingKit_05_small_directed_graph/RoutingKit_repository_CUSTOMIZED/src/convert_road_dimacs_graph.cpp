    ...
    try {
        ...
        while (std::getline(in, line)) {
            ...
            if (!was_header_read) {
                ...
                unsigned h, t, w;
                if (!(lin >> a >> t >> h >> w))
                    throw std::runtime_error("Can not parse line num " + std::to_string(line_num) + " \"" + line +
                                             "\" in dimacs file.");
                --h;
                --t;
                if (a != "a" || h >= node_count || t >= node_count)
                    throw std::runtime_error("Invalid arc in line num " + std::to_string(line_num) + " \"" + line +
                                             "\" in dimacs file.");
        ...
        if (next_arc != arc_count)
            throw std::runtime_error("The arc count in the header (" + to_string(arc_count) +
                                     ") does not correspond with the actual number of arcs (" + to_string(next_arc) +
                                     ").");
