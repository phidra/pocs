...
GraphFragment
...

    {
        auto p = compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
            node_count, fragment.tail, fragment.head);
...
CutSide BlockingFlow::get_balanced_cut() {
    ...
    std::vector<unsigned> potential_source_piercing_node(
        arc_count);  // arc_count is no typo, nodes may be multiple times in the vector
...
void BlockingFlow::advance() {
    if (!is_finished_flag && compute_blocking_flow(*fragment, is_source, is_target, is_arc_saturated, is_arc_blocked)) {
        flow_intensity +=
            augment_all_non_blocked_path(*fragment, is_source, is_target, is_arc_saturated, is_arc_blocked);
        is_finished_flag = false;
    } else {
...
CutSide inertial_flow(GraphFragment const& g,
    ...
    auto get_next_cutter = [&]() -> BlockingFlow& {
        ...
        } else if (vertical_cutter.get_current_flow_intensity() <= main_diagonal_cutter.get_current_flow_intensity() &&
                   vertical_cutter.get_current_flow_intensity() <= next_diagonal_cutter.get_current_flow_intensity()) {
            return vertical_cutter;
        } else if (main_diagonal_cutter.get_current_flow_intensity() <=
                   next_diagonal_cutter.get_current_flow_intensity()) {
        ...
        if (!c.is_finished()) {
            if (log_message) {
                ...
                if (now - last_report > 1000000) {
                    if (first_report) {
                        first_report = false;
                        log_message("Start running Inertial Flow with imbalance " + std::to_string(min_balance) +
                                    "% on graph with " + std::to_string(g.node_count()) + " nodes and " +
                                    std::to_string(g.arc_count()) + " arcs.");
                    }
                    last_report = now;
                    log_message("Smallest cutter has reached a cut of " +
                                std::to_string(c.get_current_flow_intensity()) + " arcs.");
                }
            }
            ...
            if (log_message) {
                if (!first_report) {
                    log_message("Inertial Flow is finished and needed " +
                                std::to_string(get_micro_time() - start_time) + "musec. The cut has " +
                                std::to_string(cut.cut_size) + " arcs and the smaller side has " +
                                std::to_string(cut.node_on_side_count) + " nodes.");
                }
            }
            return cut;  // NRVO
        }
...
CutSide inertial_flow(GraphFragment const& g,
    ...
    if (static_cast<unsigned long long>(c25.cut_size) * static_cast<unsigned long long>(c33.node_on_side_count) <
            static_cast<unsigned long long>(c33.cut_size) * static_cast<unsigned long long>(c25.node_on_side_count) &&
        static_cast<unsigned long long>(c25.cut_size) * static_cast<unsigned long long>(c40.node_on_side_count) <
            static_cast<unsigned long long>(c40.cut_size) * static_cast<unsigned long long>(c25.node_on_side_count))
        return c25;  // NVRO
    else if (static_cast<unsigned long long>(c33.cut_size) * static_cast<unsigned long long>(c40.node_on_side_count) <
             static_cast<unsigned long long>(c40.cut_size) * static_cast<unsigned long long>(c33.node_on_side_count))
        return c33;  // NVRO
    else
        return c40;  // NVRO
}
...
std::vector<GraphFragment> decompose_graph_fragment_into_connected_components(GraphFragment fragment) {
    ...
    auto generate_part = [&](unsigned component_node_begin,
        ...
        (void)part_node_count;
        (void)part_arc_count;

        part.tail = std::vector<unsigned>(fragment.tail.begin() + component_arc_begin,
                                          fragment.tail.begin() + component_arc_end);
        ...
        part.head = std::vector<unsigned>(fragment.head.begin() + component_arc_begin,
                                          fragment.head.begin() + component_arc_end);
        ...
        part.back_arc = std::vector<unsigned>(fragment.back_arc.begin() + component_arc_begin,
                                              fragment.back_arc.begin() + component_arc_end);
        ...
        part.global_node_id = std::vector<unsigned>(fragment.global_node_id.begin() + component_node_begin,
                                                    fragment.global_node_id.begin() + component_node_end);
...
SeparatorDecomposition compute_separator_decomposition(
    ...
    if (fragment.node_count() == 1) {
        ...
        if (log_message) {
            timer += get_micro_time();
            log_message("Finished decomposing top-level graph, needed " + std::to_string(timer) + "musec and found " +
                        std::to_string(part_list.size()) + " connected components");
        }
        ...
        for (auto& part : part_list) {
            ...
            if (part.node_count() == 1) {
                ...
                BitVector f = make_bit_vector(part.arc_count(), [&](unsigned a) {
