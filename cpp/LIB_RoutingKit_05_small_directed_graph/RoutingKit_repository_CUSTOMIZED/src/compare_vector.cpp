...
void compare_num_data(const string& vector1_file, const string& vector2_file) {
    ...
    if (vector1.size() < vector2.size()) {
        cout << "\"" << vector1_file << "\" has only " << vector1.size() << " elements while \"" << vector2_file
             << "\" has " << vector2.size() << ". Can only compare vectors of equal size." << endl;
    } else if (vector2.size() < vector1.size()) {
        cout << "\"" << vector2_file << "\" has only " << vector2.size() << " elements while \"" << vector1_file
             << "\" has " << vector1.size() << ". Can only compare vectors of equal size." << endl;
    } else {
        ...
        if (vector1_smaller_count == 0 && vector2_smaller_count == 0) {
            ...
            cout << "The vectors differ. " << vector1_smaller_count << " elements are smaller in \"" << vector1_file
                 ...
                 << "The first element that differs is at index " << first_difference << "." << endl;
        }
    }
}
...
void compare_string_data(const std::string& vector1_file, const std::string& vector2_file) {
    ...
    if (vector1.size() < vector2.size()) {
        ...
        if (equal_count == vector1.size()) {
            ...
            cout << "The vectors differ. " << equal_count << " elements are the same. " << vector1.size() - equal_count
                 << " elements are different. "
                 << "The vectors have " << vector1.size() << " elements." << endl;
        }
    }
}
