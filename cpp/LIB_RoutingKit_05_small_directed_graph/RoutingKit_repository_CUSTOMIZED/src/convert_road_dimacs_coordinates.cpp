...
int main(int argc, char* argv[]) {
    try {
        ...
        double const dimacs_scale = 1000000.0;
        ...
        while (std::getline(in, line)) {
            ...
            if (!was_header_read) {
                ...
                std::string b;
                if (!(lin >> b) || b != "p" || !(lin >> b) || b != "aux" || !(lin >> b) || b != "sp" || !(lin >> b) ||
                    b != "co")
                ...
                if (lin >> b)
                    throw std::runtime_error("The header in line " + std::to_string(line_num) +
                                             " contains extra charachters at the end of the line");
                ...
                if (seen[node])
                    throw std::runtime_error("Node " + std::to_string(node) +
                                             " has a second pair of coordinates in line " + std::to_string(line_num));
                seen[node] = true;
