...
void sort_arcs_and_remove_multi_and_loop_arcs(unsigned node_count,
    ...
    {
        auto p =
            compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(node_count, tail, head);
    ...
    if (log_message) {
        timer += get_micro_time();
        log_message("Finished removing loops and multi arcs from input. Needed " + std::to_string(timer) +
                    "musec time.");
    }
}
...
class ShorterPathTest {
   ...
   private:
    ...
    void assert_no_witness_found(unsigned len) const {
        ...
        //					assert(forward_tentative_distance[x] + backward_tentative_distance[x] >
        // len);
        //				}
        //			}
    }
    ...
    void assert_forward_tentative_distances_correct() const {
        ...
        //							if(forward_tentative_distance[y] + w ==
        // forward_tentative_distance[x]) 								witness_found =
        // true;
        //						}
        //						assert(witness_found && "forward_tentative_distance array
        // contains garbage");
        //					}
        //				}
        //			}
    ...
    void assert_backward_tentative_distances_correct() const {
        ...
        //							if(backward_tentative_distance[y] + w ==
        // backward_tentative_distance[x]) 								witness_found =
        // true;
        //						}
        //						assert(witness_found && "backward_tentative_distance array
        // contains garbage");
        //					}
        //				}
        //			}
    ...
    void assert_witness_found(unsigned len) const {
        ...
        //			for(unsigned x = 0; x<graph->node_count(); ++x){
        //				if(was_forward_pushed.is_set(x) && was_backward_pushed.is_set(x)){
        //					if(forward_tentative_distance[x] + backward_tentative_distance[x] <=
        // len) 						return;
        //				}
        //			}
   ...
   public:
    ...
    unsigned get_max_pop_count() const { return max_pop_count; }
        ...
        for (unsigned out_arc = 0; out_arc < graph.out_deg(node); ++out_arc) {
            ...
            if (in_node != out_node) {
                if (!shorter_path_test.does_shorter_or_equal_path_to_target_exist(
                        out_node, graph.in(node, in_arc).weight + graph.out(node, out_arc).weight)) {
                    ++added_arc_count;
...
void build_ch_and_order(Graph& graph,
    ...
    for (unsigned i = 0; i < node_count; ++i) {
        ...
        if (log_message) {
            ...
            if (current_time - last_log_message_time > 1000000) {
                ...
                log_message("Added " + std::to_string(i + 1) + " of " + std::to_string(node_count) +
                            " nodes to the queue. Running for " + std::to_string(timer + current_time) + "musec.");
            }
        }
    }
    ...
    std::string nameof = "ABCDEFGHIJK";

    while (!queue.empty()) {
        unsigned node_being_contracted = queue.pop().id;

        std::cout << "ABOUT TO CONTRACT NEW NODE !" << std::endl;
        std::cout << "node is : " << node_being_contracted << " = " << nameof[node_being_contracted] << std::endl;
        std::cout << "its rank is : " << contracted_node_count << std::endl;
        std::cout << "note : BEFORE contracting it, we add its edges :" << std::endl;
        ...
        for (unsigned in_arc = 0; in_arc < graph.in_deg(node_being_contracted); ++in_arc) {
            ...
            std::cout << "\t- adding BACKWARD arc between " << nameof[a.node] << " and "
                      << nameof[node_being_contracted] << " of weight " << a.weight << std::endl;
        ...
        if (log_message) {
            ...
            if (current_time - last_log_message_time > 1000000) {
                last_log_message_time = current_time;
                log_message("Contracted " + std::to_string(contracted_node_count) + " of " +
                            std::to_string(node_count) + ". The in degree of last node was " + std::to_string(in_deg) +
                            " and out degree was " + std::to_string(out_deg) + ". Running for " +
                            std::to_string(timer + current_time) + "musec.");
            }
        }
        std::cout << std::endl;
    }
...
void build_ch_given_rank(Graph& graph,
    ...
    for (unsigned i = 0; i < node_count; ++i) {
        ...
        if (log_message) {
            ...
            if (current_time - last_log_message_time > 1000000) {
                last_log_message_time = current_time;
                log_message("Contracted " + std::to_string(i + 1) + " of " + std::to_string(node_count) +
                            ". The in degree of last node was " + std::to_string(in_deg) + " and out degree was " +
                            std::to_string(out_deg) + ". Running for " + std::to_string(timer + current_time) +
                            "musec.");
            }
        }
    }
...
void sort_ch_arcs_and_build_first_out_arrays(ContractionHierarchy& ch,
    ...
    {
        auto r = compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
            node_count, ch_extra.forward.tail, ch.forward.head);
    ...
    {
        auto r = compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(
            node_count, ch_extra.backward.tail, ch.backward.head);
...
void optimize_order_for_cache(ContractionHierarchy& ch,
    ...
    std::string nameof = "ABCDEFGHIJK";
    std::cout << std::endl;
    std::cout << "=== REORDORING NODES TO BE CACHE-FRIENDLY :" << std::endl;
    std::cout << "OLD ORDER IS : " << std::endl;
    ...
    for (unsigned x = 0; x < node_count; ++x) {
        for (unsigned xy = ch.forward.first_out[x]; xy < ch.forward.first_out[x + 1]; ++xy) {
            ...
            if (z == invalid_id) {
                ...
                ch.forward.shortcut_first_arc[xy] =
                    find_arc_given_sorted_head(ch.backward.first_out, ch.backward.head, z, x);
                ch.forward.shortcut_second_arc[xy] =
                    find_arc_given_sorted_head(ch.forward.first_out, ch.forward.head, z, y);

                assert(ch.forward.weight[xy] == ch.backward.weight[ch.forward.shortcut_first_arc[xy]] +
                                                    ch.forward.weight[ch.forward.shortcut_second_arc[xy]]);
            }
        }
    }
    ...
    for (unsigned x = 0; x < node_count; ++x) {
        for (unsigned xy = ch.backward.first_out[x]; xy < ch.backward.first_out[x + 1]; ++xy) {
            ...
            if (z == invalid_id) {
                ...
                ch.backward.shortcut_first_arc[xy] =
                    find_arc_given_sorted_head(ch.backward.first_out, ch.backward.head, z, y);
                ch.backward.shortcut_second_arc[xy] =
                    find_arc_given_sorted_head(ch.forward.first_out, ch.forward.head, z, x);
            }
        }
    }
...
void log_input_graph_statistics(unsigned node_count,
    ...
    if (log_message) {
        ...
        log_message("The input's maximum in-degree is " + std::to_string(max_in_degree) +
...
ContractionHierarchy ContractionHierarchy::build(unsigned node_count,
                                                 std::vector<unsigned> tail,
                                                 std::vector<unsigned> head,
                                                 std::vector<unsigned> weight,
                                                 const std::function<void(std::string)>& log_message,
...
ContractionHierarchy ContractionHierarchy::build_given_rank(std::vector<unsigned> rank,
                                                            std::vector<unsigned> tail,
                                                            std::vector<unsigned> head,
                                                            std::vector<unsigned> weight,
                                                            const std::function<void(std::string)>& log_message,
...
ContractionHierarchy ContractionHierarchy::build_given_order(std::vector<unsigned> order,
                                                             std::vector<unsigned> tail,
                                                             std::vector<unsigned> head,
                                                             std::vector<unsigned> weight,
                                                             const std::function<void(std::string)>& log_message,
...
void check_contraction_hierarchy_for_errors(const ContractionHierarchy& ch) {
    ...
    for (unsigned x = 0; x < node_count; ++x) {
        for (unsigned xy = ch.forward.first_out[x]; xy < ch.forward.first_out[x + 1]; ++xy) {
            unsigned y = ch.forward.head[xy];
            if (y <= x)
                throw std::runtime_error("CH is invalid because: forward graph contains downward arc " +
                                         std::to_string(x) + " -> " + std::to_string(y));
        }
        for (unsigned xy = ch.backward.first_out[x]; xy < ch.backward.first_out[x + 1]; ++xy) {
            unsigned y = ch.backward.head[xy];
            if (y <= x)
                throw std::runtime_error("CH is invalid because: backward graph contains downward arc " +
                                         std::to_string(x) + " -> " + std::to_string(y));
        }
    }
    ...
    for (unsigned xy = 0; xy < forward_arc_count; ++xy) {
        if (!ch.forward.is_shortcut_an_original_arc.is_set(xy)) {
            if (ch.forward.shortcut_first_arc[xy] >= backward_arc_count)
                throw std::runtime_error("CH is invalid because: ch.forward.shortcut_first_arc[" + std::to_string(xy) +
                                         "] >= backward_arc_count");
            if (ch.forward.shortcut_second_arc[xy] >= forward_arc_count)
                throw std::runtime_error("CH is invalid because: ch.forward.shortcut_second_arc[" + std::to_string(xy) +
                                         "] >= forward_arc_count");
            if (ch.forward.shortcut_second_arc[xy] >= xy)
                throw std::runtime_error("CH is invalid because: ch.forward.shortcut_second_arc[" + std::to_string(xy) +
                                         "] >= " + std::to_string(xy));
            if (ch.forward.weight[xy] != ch.backward.weight[ch.forward.shortcut_first_arc[xy]] +
                                             ch.forward.weight[ch.forward.shortcut_second_arc[xy]])
                throw std::runtime_error(
                    "CH is invalid because: ch.forward.weight[xy] != "
                    "ch.backward.weight[ch.forward.shortcut_first_arc[xy]] + "
                    "ch.forward.weight[ch.forward.shortcut_second_arc[xy]]");
        } else {
    ...
    for (unsigned xy = 0; xy < backward_arc_count; ++xy) {
        if (!ch.backward.is_shortcut_an_original_arc.is_set(xy)) {
            if (ch.backward.shortcut_first_arc[xy] >= backward_arc_count)
                throw std::runtime_error("CH is invalid because: ch.backward.shortcut_first_arc[" + std::to_string(xy) +
                                         "] >= backward_arc_count");
            if (ch.backward.shortcut_second_arc[xy] >= forward_arc_count)
                throw std::runtime_error("CH is invalid because: ch.backward.shortcut_second_arc[" +
                                         std::to_string(xy) + "] >= forward_arc_count");
            if (ch.backward.shortcut_first_arc[xy] >= xy)
                throw std::runtime_error("CH is invalid because: ch.backward.shortcut_first_arc[" + std::to_string(xy) +
                                         "] >= " + std::to_string(xy));
            if (ch.backward.weight[xy] != ch.backward.weight[ch.backward.shortcut_first_arc[xy]] +
                                              ch.forward.weight[ch.backward.shortcut_second_arc[xy]])
                throw std::runtime_error(
                    "CH is invalid because: ch.backward.weight[xy] != "
                    "ch.backward.weight[ch.backward.shortcut_first_arc[xy]] + "
                    "ch.forward.weight[ch.backward.shortcut_second_arc[xy]]");
        } else {
            ...
            if (ch.backward.shortcut_second_arc[xy] >= node_count)
                throw std::runtime_error("CH is invalid because: ch.backward.shortcut_second_arc[" +
                                         std::to_string(xy) + "] >= node_count for an original arc");
        }
    }
}
...
ContractionHierarchy ContractionHierarchy::read(std::istream& in) {
    return read([&](char* p, unsigned long long l) {
        if (!in.read(p, l))
...
ContractionHierarchy ContractionHierarchy::read(std::function<void(char*, unsigned long long)> in,
    ...
    unsigned long long expected_file_size =
        (sizeof(CHFileHeader) +
         sizeof(unsigned) * (header.node_count +
                             (header.node_count + 1 + 4 * header.forward_arc_count

                              ) +
                             (header.node_count + 1 + 4 * header.backward_arc_count

                              )) +
         ((header.backward_arc_count + 511) / 512) * 64 + ((header.forward_arc_count + 511) / 512) * 64);
...
void forward_settle_node(unsigned& shortest_path_length,
    ...
    std::cout << "\t settled node's rank is : " << popped_node << std::endl;
    ...
    if (was_backward_pushed.is_set(popped_node)) {
        if (shortest_path_length > distance_to_popped_node + backward_tentative_distance[popped_node]) {
            ...
            std::cout << "\t====== FOUND A MEETING NODE, ITS RANK IS : " << shortest_path_meeting_node << std::endl;
        }
    }
    ...
    if (shortest_path_meeting_node != invalid_id) {
        std::vector<unsigned> up_path;
        {
            ...
            while (forward_predecessor_node[x] != invalid_id) {
                ...
                // up_path.push_back(find_arc_given_sorted_head(ch->forward.first_out, ch->forward.head,
                // forward_predecessor_node[x], x));
                x = forward_predecessor_node[x];
            }
        }
...
void pinned_run(std::vector<unsigned>& select_list,
                unsigned& select_count,
