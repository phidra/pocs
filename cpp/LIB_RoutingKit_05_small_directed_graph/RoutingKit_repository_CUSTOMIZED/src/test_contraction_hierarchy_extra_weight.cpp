...
int main(int argc, char* argv[]) {
    try {
        ...
        {
            unsigned node_count = 4;
            std::vector<unsigned> tail = {0, 0, 1, 2}, head = {1, 2, 3, 3}, weight = {1, 10, 1, 10},
                                  extra_weight1 = {10, 1, 10, 1};
            std::vector<int> extra_weight2 = {-10, -1, -10, -1};
            std::vector<std::string> extra_weight3 = {"foo", "bla", "bar", "hoo"};
            auto ch = ContractionHierarchy::build(node_count, tail, head, weight);
            ContractionHierarchyQuery q(ch);
...

        {
            ...
            std::vector<unsigned> tail = {0, 0, 1, 2}, head = {1, 2, 3, 3}, weight = {1, 10, 1, 10},
                                  target_list = {3, 1};
            std::vector<std::string> extra_weight = {"foo", "bla", "bar", "hoo"};

            auto ch = ContractionHierarchy::build(node_count, tail, head, weight);
            ContractionHierarchyQuery q(ch);

            std::vector<std::string> d = q.pin_targets(target_list)
                                             .add_source(0)
                                             .run_to_pinned_targets()
                                             .get_extra_weight_distances_to_targets(
                                                 extra_weight, [](std::string l, std::string r) { return l + r; });
...

        {
            ...
            for (unsigned s = 0; s < node_count; ++s) {
                for (unsigned t = 0; t < node_count; ++t) {
                    ...
                    unsigned computed_extra_scalar_weight =
                        q.get_extra_weight_distance(ch_extra_scalar_weight, scalar_link);

                    EXPECT_CMP(correct_extra_scalar_weight, ==, computed_extra_scalar_weight);

                    vector<unsigned> computed_extra_path_weight =
                        q.get_extra_weight_distance(ch_extra_path_weight, path_link);
        ...
        if (argc == 5) {
            vector<unsigned> first_out = load_vector<unsigned>(argv[1]), tail = invert_inverse_vector(first_out),
...

            {
                for (unsigned t = 0; t < test_count; ++t) {
                    ...
                    auto computed_extra_scalar_weight =
                        q.get_extra_weight_distance(ch_extra_scalar_weight, scalar_link);
                    EXPECT_CMP(correct_extra_scalar_weight, ==, computed_extra_scalar_weight);
                ...
                for (unsigned k = 0; k < target_test_count - 3; ++k) {
                    ...
                    for (unsigned i = 0; i < target_test_count; ++i) {
                        ...
                        if (q.get_distance() == inf_weight) {
                            ...
                            EXPECT(test_node_list[k + 0] == t || test_node_list[k + 1] == t ||
                                   test_node_list[k + 2] == t);
                            EXPECT_CMP(scalar_result[i], ==, 0);
                        } else {
                ...
                for (unsigned k = 0; k < target_test_count - 3; ++k) {
                    ...
                    for (unsigned i = 0; i < target_test_count; ++i) {
                        ...
                        if (q.get_distance() == inf_weight) {
                            ...
                            EXPECT(test_node_list[k + 0] == s || test_node_list[k + 1] == s ||
                                   test_node_list[k + 2] == s);
                            EXPECT_CMP(scalar_result[i], ==, 0);
                        } else {
                            EXPECT_CMP(tail[path_result[i].front()], ==, s);
...

            {
                ...
                p.reset().pin_targets(test_node_list);
                for (unsigned k = 0; k < target_test_count; ++k) {
                    p.reset_source().add_source(test_node_list[k]).run_to_pinned_targets();
                ...
                cout << "One average scalar one-to-" << target_test_count
                     << " get_extra_weight_distances_to_targets needs " << timer / target_test_count << " musec"
                     << endl;
            }

            {
                ...
                cout << "One average scalar one-to-" << target_test_count
                     << " get_extra_weight_distances_to_targets with on-demand unpacking needs "
                     << timer / target_test_count << " musec" << endl;
            }

            {
                ...
                cout << "One average scalar three-to-" << target_test_count
                     << " get_extra_weight_distances_to_targets needs " << timer / (target_test_count - 3) << " musec"
                     << endl;
            }

            {
                ...
                cout << "One average path one-to-" << target_test_count
                     << " get_extra_weight_distances_to_targets needs " << timer / target_test_count << " musec"
                     << endl;
            }
...

            {
                ...
                cout << "One average path three-to-" << target_test_count
                     << " get_extra_weight_distances_to_targets needs " << timer / (target_test_count - 3) << " musec"
                     << endl;
            }
