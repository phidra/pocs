    ...
    try {
        {
            ...
            char file_char = 0;
            function<unsigned long long(char*, unsigned long long)> data_source =
                [&](char* buffer, unsigned long long amount_to_read) {
...

        {
            ...
            if (total_bytes_read != file_length)
                throw runtime_error("file_length is " + to_string(file_length) + " but " + to_string(total_bytes_read) +
                                    " bytes were read");
