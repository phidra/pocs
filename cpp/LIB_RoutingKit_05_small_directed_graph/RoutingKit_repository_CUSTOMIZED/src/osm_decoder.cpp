...
static uint32_t ntohl(uint32_t netlong) {
    return (netlong << 24) | ((netlong << 16) >> 8) | ((netlong << 8) >> 16) | (netlong >> 24);
}
#endif
   ...
   public:
    ...
    std::function<unsigned long long(char*, unsigned long long)> get_read_function_object() {
        return [&](char* buffer, unsigned long long how_much_to_read) -> unsigned long long {
                        ...
                        [&](uint64_t key_id, const char* str_begin, const char* str_end) {
                            if (key_id == 4) {  // must support
                                if (!std::equal(str_begin, str_end, "DenseNodes"))
                                    throw std::runtime_error("Required OSM PBF feature \"" +
                                                             std::string(str_begin, str_end) + "\" is unknown");
            ...
            const char *uncompressed_begin = nullptr, *uncompressed_end = nullptr, *compressed_begin = nullptr,
            ...
            if (uncompressed_begin) {
                if (uncompressed_data_size > how_much_to_read - 4)
                    throw std::runtime_error("PBF error: Blob is too large. It is " +
                                             std::to_string(uncompressed_data_size) + " but may be at most " +
                                             std::to_string(how_much_to_read - 4));
                if (uncompressed_data_size != (std::uint64_t)(uncompressed_end - uncompressed_begin))
                    throw std::runtime_error(
                        "PBF error: claimed uncompressed blob size does not correspond to actual blob size");
                ...
                if (inflate(&z, Z_FINISH) != Z_STREAM_END) {
                    throw std::runtime_error(
                        "PBF error: Failed to completely inflate zlib stream. Probably the OSM blob decompresses to "
                        "something larger than reported in the header.");
                }
                ...
                if (z.total_out != uncompressed_data_size) {
                    throw std::runtime_error(
                        "PBF error: OSM blob decompresses to fewer bytes than reported in the header.");
                }
                return uncompressed_data_size + 4;
            }
        };
    ...
    for (;;) {
        ...
        auto decode_dense_node = [&](const char* begin, const char* end) {
            ...
            while (osm_node_id_begin != osm_node_id_end) {
                osm_node_id += zigzag_convert_uint64_to_int64(
                    decode_varint_as_uint64_and_advance_first_parameter(osm_node_id_begin, osm_node_id_end));
                latitude += primblock_lon_offset +
                            primblock_granularity *
                                zigzag_convert_uint64_to_int64(
                                    decode_varint_as_uint64_and_advance_first_parameter(latitude_begin, latitude_end));
                longitude += primblock_lon_offset +
                             primblock_granularity *
                                 zigzag_convert_uint64_to_int64(decode_varint_as_uint64_and_advance_first_parameter(
                                     longitude_begin, longitude_end));
                if (key_value_pairs_begin != nullptr) {
                    key_list.clear();
                    value_list.clear();
                    for (;;) {
                        uint64_t x = decode_varint_as_uint64_and_advance_first_parameter(key_value_pairs_begin,
                                                                                         key_value_pairs_end);
                        if (x == 0)
                            break;
                        uint64_t y = decode_varint_as_uint64_and_advance_first_parameter(key_value_pairs_begin,
                                                                                         key_value_pairs_end);
            ...
            if (latitude_begin != latitude_end)
                throw std::runtime_error(
                    "PBF error: dense node latitude array has a different length than the node ID array.");
            if (longitude_begin != longitude_end)
                throw std::runtime_error(
                    "PBF error: dense node longitude array has a different length than the node ID array.");
        ...
        auto decode_way = [&](const char* begin, const char* end) {
            ...
            while (node_list_begin != node_list_end) {
                id += zigzag_convert_uint64_to_int64(
                    decode_varint_as_uint64_and_advance_first_parameter(node_list_begin, node_list_end));
        ...
        auto decode_relation = [&](const char* begin, const char* end) {
            ...
            const char *key_begin = nullptr, *key_end = nullptr, *value_begin = nullptr, *value_end = nullptr,
                       *member_role_begin = nullptr, *member_role_end = nullptr, *member_id_begin = nullptr,
            ...
            while (member_id_begin != member_id_end) {
                member_id += zigzag_convert_uint64_to_int64(
                    decode_varint_as_uint64_and_advance_first_parameter(member_id_begin, member_id_end));
                ...
                uint64_t type_id =
                    decode_varint_as_uint64_and_advance_first_parameter(member_type_begin, member_type_end);
...
void ordered_read_osm_pbf(
    ...
    if (file_is_ordered_even_though_file_header_says_that_it_is_unordered ||
        (decompressor.get_status() & is_ordered_bit) != 0) {
        internal_read_osm_pbf(reader, node_callback, way_callback, relation_callback, log_message);
    } else {
        if (node_callback) {
...
void speedtest_osm_pbf_reading(const std::string& pbf_file, std::function<void(std::string)> log_message) {
    ...
    auto produce_report = [&]() {
        ...
        if (now - last_report > 1000000) {
            ...
            out << "Read " << std::setw(9) << node_count << " nodes, " << std::setw(9) << way_count << " ways, "
                << std::setw(9) << rel_count << " relations last second";
            log_message(out.str());
            last_report = now;
            node_count = 0;
            way_count = 0;
