...
int main() {
    {
        ...
        for (unsigned range : range_list) {
            ...
            EXPECT(q ==
                   compute_stable_sort_permutation_using_comparator(v, [](unsigned l, unsigned r) { return l < r; }));
        }
    }

    {
        ...
        for (unsigned range : range_list) {
            ...
            EXPECT(q == compute_inverse_stable_sort_permutation_using_comparator(
                            v, [](unsigned l, unsigned r) { return l < r; }));
        }
    }

    {
        ...
        for (unsigned range : range_list) {
            ...
            EXPECT(is_sorted_using_less(
                apply_permutation(compute_sort_permutation_using_key(v, range, [](unsigned x) { return x; }), v)));
            EXPECT(is_sorted_using_less(apply_permutation(compute_sort_permutation_using_less(v), v)));
            EXPECT(is_sorted_using_less(apply_permutation(
                compute_sort_permutation_using_comparator(v, [](unsigned l, unsigned r) { return l < r; }), v)));
        }
    }

    {
        ...
        for (unsigned range : range_list) {
            ...
            EXPECT(is_sorted_using_less(apply_inverse_permutation(
                compute_inverse_sort_permutation_using_key(v, range, [](unsigned x) { return x; }), v)));
            EXPECT(is_sorted_using_less(apply_inverse_permutation(compute_inverse_sort_permutation_using_less(v), v)));
            EXPECT(is_sorted_using_less(apply_inverse_permutation(
                compute_inverse_sort_permutation_using_comparator(v, [](unsigned l, unsigned r) { return l < r; }),
                v)));
        }
    }

    {
        ...
        EXPECT(sorted_v == apply_permutation(compute_sort_permutation_using_key(v, key_count, get_key), v));

        EXPECT(sorted_v ==
               apply_inverse_permutation(compute_inverse_sort_permutation_using_key(v, key_count, get_key), v));

        EXPECT(sorted_v == apply_permutation(compute_stable_sort_permutation_using_key(v, key_count, get_key), v));

        EXPECT(sorted_v ==
               apply_inverse_permutation(compute_inverse_stable_sort_permutation_using_key(v, key_count, get_key), v));

        EXPECT(is_sorted_using_key(sorted_v, key_count, get_key));

        EXPECT(sorted_v == apply_permutation(compute_sort_permutation_using_comparator(v, less_comparator), v));

        EXPECT(sorted_v == apply_permutation(compute_stable_sort_permutation_using_comparator(v, less_comparator), v));

        EXPECT(sorted_v == apply_permutation(compute_sort_permutation_using_less(v), v));

        EXPECT(sorted_v == apply_permutation(compute_stable_sort_permutation_using_less(v), v));

        EXPECT(is_sorted_using_less(sorted_v));

        EXPECT(is_sorted_using_comparator(sorted_v, less_comparator));

        std::reverse(sorted_v.begin(), sorted_v.end());

        EXPECT(sorted_v == apply_permutation(compute_sort_permutation_using_comparator(v, greater_comparator), v));

        EXPECT(sorted_v ==
               apply_permutation(compute_stable_sort_permutation_using_comparator(v, greater_comparator), v));

        EXPECT(is_sorted_using_comparator(sorted_v, greater_comparator));
    }

    {
        ...
        auto p2 =
            compute_stable_sort_permutation_using_key(identity_permutation(100), 100, [&](unsigned x) { return v[x]; });
...

    {
        ...
        EXPECT(sorted_v == apply_permutation(compute_stable_sort_permutation_using_key(v, key_count, get_x_key), v));

        EXPECT(sorted_v == apply_inverse_permutation(
                               compute_inverse_stable_sort_permutation_using_key(v, key_count, get_x_key), v));

        EXPECT(sorted_v ==
               apply_permutation(compute_stable_sort_permutation_using_comparator(v, x_less_comparator), v));
    }
...
*/
        ...
        time = -get_micro_time();
        foo = apply_inverse_permutation(
            compute_inverse_sort_permutation_using_key(elements, key_count, [](unsigned x) { return x; }), elements);
        time += get_micro_time();
        print_something_to_prevent_optimizer_from_removing_everything(foo);
