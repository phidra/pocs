...
inline double geo_dist(double lat_a, double lon_a, double lat_b, double lon_b) {
    ...
    // fourth dummy value.
    //
    // If the code was vectorized then the compiler would have to insert this dummy value implicitly. We are helping it
    // by doing it explicitly. If SIMD is disabled, the array should be converted by the optimizer to 4 variables. The
    // dummy variable should subsequently be eliminated using dead code elimination.
    double vec[4] = {lat_a - lat_b, lat_a + lat_b, lon_a - lon_b, 0};
