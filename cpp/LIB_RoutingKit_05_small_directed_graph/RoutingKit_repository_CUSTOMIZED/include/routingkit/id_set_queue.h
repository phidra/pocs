...
class IDSetMinQueue {
   private:
    ...
    static unsigned other_child(unsigned x) { return x ^ 1; }

    //! True if other_child(x) < x. Only works if x != root.
    static unsigned is_larger_child(unsigned x) { return x & 1; }

    //! True if other_child(x) > x. Only works if x != root.
    static unsigned is_smaller_child(unsigned x) { return !is_larger_child(x); }

    //! Returns the node y such that parent(y) == x and y < other_child(y).
    static unsigned smaller_child(unsigned x) { return x << 1; }

    //! Returns the node y such that parent(y) == x and y > other_child(y).
    static unsigned larger_child(unsigned x) { return (x << 1) | 1; }
