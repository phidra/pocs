...
//! A priority queue where the elements are IDs from 0 to id_count-1 where id_count is a number that is set in the
...
class MinIDQueue {
   ...
   public:
    ...
    explicit MinIDQueue(unsigned id_count) : id_pos(id_count, invalid_id), heap(id_count), heap_size(0) {}
