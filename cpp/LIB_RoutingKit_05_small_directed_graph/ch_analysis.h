...
struct ContractedEdge {
    ...
    static inline ContractedEdge create(RoutingKit::ContractionHierarchy const& ch, RoutingKit::ContractionHierarchy::Side const& graph, unsigned edge_id) {
        ContractedEdge edge;
...
std::tuple<unsigned, unsigned, bool> get_edge_info(unsigned xy, RoutingKit::ContractionHierarchy const& ch, Direction dir) {
...
void _out_edges_of(OriginalGraph const& original_graph, unsigned x_rank, RoutingKit::ContractionHierarchy const & ch, TNameOf const & nameof, Direction dir) {
    ...
    for (unsigned xy = graph.first_out[x_rank]; xy < graph.first_out[x_rank+1]; ++xy) {
        ...
        std::cout << "\t[id=" << xy << "]  " << dir_string << original_string << nameof(x_node) << nameof(edge.head_id) << " has weight " << edge.weight << std::endl;
        if (! edge.is_original) {
            ...
            // NOTE 1 = premier point important à comprendre : first_half_edge_id et second_half_edge_id (qui sont des index d'edges) ne réfèrent pas à la même structure !
            //      - shortcut_first_arc est un edge-index dans ch.backward
            //      - shortcut_second_arc est un edge-index dans ch.forward
            // Ceci découle du fait que les rank d'un shortcut AB (ayant pour middle-node X) sont nécessairement X < A < B :
            ...
            // NOTE 2 : si un (demi-)edge est un edge ORIGINAL, alors shortcut_first_arc et shortcut_second_arc se réfèrent au graphe ORIGINAL :
            ...
            // Le graphe original dont il est question est celui passé à ContractionHierarchy::build (le triplet head+tail/weight).
            // Et comme il n'est pas stocké dans la structure ContractionHierarchy, interpréter shortcut_first/second_arc pour un edge original NÉCESSITE le graphe original
            // (c'est pour ça qu'il est passé en argument de la présente fonction)

            // Le code ci-dessous montre que shortcut_first_arc est l'edge original, et shortcut_second_arc est le tail de celui-ci :
            // Je ne peux illustrer ceci qu'aver le PREMIER demi-edge, car pour le second, il y a (je pense) un bug dans RoutingKit...
            if (half_edge_1.is_original) {
                ...
                // Le tail d'un edge backward est en fait un head dans le graphe original (donc on utilise original_graph.heads) :
            ...
            std::cout << "\t\tJe m'intéresse au PREMIER demi-edge de ce shortcut (c'est donc un edge du graphe BACKWARD), qui a pour backward-id : " << edge.first_half_edge_id << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour poids               : " << half_edge_1.weight << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour head                : " << half_edge_1.head_rank << " = " << nameof(half_edge_1.head_id) << std::endl;
            std::cout << "\t\t\tCe demi-edge est-il original            ? " << std::boolalpha << half_edge_1.is_original << std::endl;
            // NOTE : comme mon graphe est simpliste, un demi-edge sera TOUJOURS original, les shortcut_first/second_arc des demi-arcs représentent donc un edge réel, ce que j'exploite (en affichant l'edge/tail original) :
            std::cout << "\t\t\tCe demi-edge a pour shortcut_first_arc  : " << half_edge_1.first_half_edge_id << " = " << original_graph.edgeinfo(half_edge_1.first_half_edge_id) << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour shortcut_second_arc : " << half_edge_1.second_half_edge_id << " = " << nameof(half_edge_1.second_half_edge_id) << std::endl;

            std::cout << "\t\tJe m'intéresse au DEUXIEME demi-edge de ce shortcut (c'est donc un edge du graphe FORWARD), qui a pour forward-id : " << edge.second_half_edge_id << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour poids               : " << half_edge_2.weight << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour head                : " << half_edge_2.head_rank << " = " << nameof(half_edge_2.head_id) << std::endl;
            std::cout << "\t\t\tCe demi-edge est-il original            ? " << std::boolalpha << half_edge_2.is_original << std::endl;
            // NOTE : comme mon graphe est simpliste, un demi-edge sera TOUJOURS original, les shortcut_first/second_arc des demi-arcs représentent donc un edge réel, ce que j'exploite (en affichant l'edge/tail original) :
            std::cout << "\t\t\tCe demi-edge a pour shortcut_first_arc  : " << half_edge_2.first_half_edge_id << " = " << original_graph.edgeinfo(half_edge_2.first_half_edge_id) << std::endl;
            std::cout << "\t\t\tCe demi-edge a pour shortcut_second_arc : " << half_edge_2.second_half_edge_id << " = " << nameof(half_edge_2.second_half_edge_id) << std::endl;
            std::cout << "\t\t\t                                          ^  CECI N'EST PAS LE TAIL DU DEMI-EDGE (mais son head), CAR IL Y A UN BUG DANS RoutingKit..." << std::endl;
            ...
            // le seul cas où on peut le faire (ce qui est mon cas ici vu la simplicité de mon graphe) est celui où l'un des deux demi-edges est un edge ORIGINAL
            // en effet, dans ce cas, j'ai accès (via shortcut_first/second_arc) à toutes les infos de l'edge original, y compris son tail
            // (en pratique, il y a selon moi un bug dans routing-kit, et je n'ai directemetn accès au tail du node que pour le SECOND demi-edge)
        }
    }
}
...
template <typename TNameOf>
void analyze_graph(OriginalGraph const& original_graph, RoutingKit::ContractionHierarchy const& ch, Direction dir, TNameOf const& nameof) {
...
template <typename TNameOf>
void display_2_forward_graph(OriginalGraph const& original_graph, RoutingKit::ContractionHierarchy const& ch, TNameOf const& nameof) {
...
template <typename TNameOf>
void analyze_ch(OriginalGraph const& original_graph, RoutingKit::ContractionHierarchy const& ch, TNameOf const& nameof) {
