...
concept init_list_constructible = std::constructible_from < std::decay_t<T>,
...
struct any {
    ...
    template <class T>
    requires(!std::same_as<std::decay_t<T>, any> && !is_in_place_type<T> && copy_constructible<T>) any(T&& value) {
        emplace<std::decay_t<T>>(std::forward<T>(value));
    }

    template <class T, class... Args>
    requires(std::constructible_from<std::decay_t<T>, Args...>&& copy_constructible<T>) explicit any(
    ...
    template <class T, class List, class... Args>
    requires(init_list_constructible<T, List, Args...>&& copy_constructible<T>) explicit any(
        std::in_place_type_t<T>,
    ...
    template <class T>
    requires(!std::is_same_v<std::decay_t<T>, any> && copy_constructible<T>) any& operator=(T&& rhs) {
        any(std::forward<T>(rhs)).swap(*this);
        return *this;
    }

    template <class T, class... Args>
    requires(std::constructible_from<std::decay_t<T>, Args...>&& copy_constructible<T>) std::decay_t<T>
    &emplace(Args&&... args) {
    ...
    template <class T, class List, class... Args>
    requires(init_list_constructible<T, List, Args...>&& copy_constructible<T>) std::decay_t<T>
...
template <class T>
requires(std::constructible_from<T, remove_cvref_t<T>&>) T any_cast(any& a) {
    using V = remove_cvref_t<T>;
    if (auto* value = any_cast<V>(&a)) {
        return static_cast<T>(*value);
    } else {
