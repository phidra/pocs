...
struct request_t {
    ...
    struct glaze {
        using T = request_t;
        static constexpr auto value = glz::object("jsonrpc",
                                                  &T::version,  //
                                                  "method",
                                                  &T::method,  //
                                                  "params",
...
concept call_return_type = requires {
    requires glz::is_any_of<call_return_t, std::string, std::vector<response_t<glz::raw_json>>>;
};
}  // namespace concepts
            ...
            if (auto it{method.pending_requests.find(res.id)}; it != std::end(method.pending_requests)) {
                ...
                if (!response.has_value()) {
                    ...
                    if (res_typed.result.has_value()) {
                        ...
                        return_v =
                            rpc::error{rpc::error_e::parse_error, R"(Missing key "result" or "error" in response)"};
                    }
                }
