...
int main(void) {
    ...
    check_file("test05_finalcomma_middle.txt",
               {{"luke", "35", "AAA"}, {"padme", "62", "BBB", ""}, {"anakin", "55", "CCC"}});
    check_file("test06_commathenempty_end.txt",
               {{"luke", "35", "AAA"}, {"padme", "62", "BBB"}, {"anakin", "55", "CCC", ""}, {""}});
    check_file("test07_commathenempty_middle.txt",
               {{"luke", "35", "AAA"}, {"padme", "62", "BBB", ""}, {""}, {"anakin", "55", "CCC"}});
    check_file("test08_emptyfile.txt", {});  // aucun caractère dans le fichier -> parsé comme une seule ligne vide
    check_file("test09_emptyline.txt",
               {{""}});  // un seul caractère dans le fichier (\n) -> parsé comme deux lignes vides
