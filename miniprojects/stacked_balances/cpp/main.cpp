...
// supposition à mentionner explicitement : un seul arbre pour les balances (il n'y a pas plusieurs composantes disjointes)
// supposition à mentionner explicitement : la balance 0 est forcément la racine de l'arbre
// supposition à mentionner explicitement : fichier d'entrée bien formé
// edge-case auquel on est robuste = quand il y a plusieurs façons d'équilibrer les balances
...
struct Plate
{
    ...
    {
        return accumulate(
            balances_indexes.begin(),
            balances_indexes.end(),
            string(""),
...
struct DebuggingVisitor : public Visitor
{
    ...
    void visit(Balance& visited) override
    {
        ...
        cout << "Left  plate (initial=" << visited.left.initial_weight << ") has " << visited.left.added_weight << " added" << endl;
        cout << "Right plate (initial=" << visited.right.initial_weight << ") has " << visited.right.added_weight << " added" << endl;
