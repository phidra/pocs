...
DEFAULT_OUTPUT_DIR="/tmp/propagation25/"
...
DESCRIPTION = (dirtily) compute shortest path using CH bidirectinoal propagation

(dirtily because a lot ot things in the code are hardcoded / unexplained)

In fine, shortest path is computed in 4 different ways :
    - via a call to networkx.shortest_path(), this is the reference path
    - using a unidirectional dijkstra
    - using a bidirectional dijkstra
    - using CH (which also uses bidirectional dijkstra)

Some asserts ensure that all shortest path are identical :-)
