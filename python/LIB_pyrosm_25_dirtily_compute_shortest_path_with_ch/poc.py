...
def run_bidir_dijkstra(orig_graph, source_osmid, target_osmid, output_dir):
    # drawer = DijkstraDrawer(output_dir, "image", save_period=10, max_nb_file=9)
    # drawer.plot_graph(orig_graph.graph, **PlotConf.NO_NODE, **PlotConf.LIGHT_EDGES)  # background graph
    drawer = DummyDrawer()
...
def run_ch_query(orig_graph, source_osmid, target_osmid, output_dir):
    ...
    bidir_dijkstra = BidirDijkstra(
        ch.forward,
        ch.backward,
        event_handler_forward=dummy_drawer,
        event_handler_backward=dummy_drawer,
        weight_attr="length",
    )
    forward_path, joining_node, backward_path = bidir_dijkstra.run(source=source_osmid, target=target_osmid)
