...
def get_new_ax(fig, ax):
    """ adds a new ax, and configures it to properly integrate with existing figure"""
    new_ax = fig.add_axes(ax.get_position(), sharex=ax, sharey=ax)
...
def draw_nb_settled_nodes(fig, ax, nb_settled_nodes):
    # https://matplotlib.org/stable/api/_as_gen/matplotlib.axes.Axes.text.html#matplotlib.axes.Axes.text
    nb_settled_ax = get_new_ax(fig, ax)
    text = "nb settled nodes = {}".format(nb_settled_nodes)
    nb_settled_ax.text(0.02, 0.9, text, transform=ax.transAxes)
    return nb_settled_ax


def draw_subgraph(output_dir, fig, ax, G, nodes, edges, file_counter, hull, nb_settled):
    ...
    # drawing "temporary" axes (the ones that are redrawn at each iteration) :
    hull_ax = draw_hull(fig, ax, hull)
    nb_settled_ax = draw_nb_settled_nodes(fig, ax, nb_settled)
    ...
    hull_ax.remove()
    nb_settled_ax.remove()

    return dumped


def plot_nodes(output_dir, fig, ax, G, parents, nodes, file_counter, hull, nb_settled):
    """
    ...
    draw_subgraph(output_dir, fig, ax, G, nodes_to_draw, edges_to_draw, file_counter, hull, nb_settled)
