...
# NOTE 1 : when compared to the previous POC, these two classes are new.
# We define a type `HandlerInterface` to represent all possible handlers.
# We use a metaclass `HandlerMeta` to define this interface ("it must implement a 'on_iteration_start' method")
...
class HandlerInterface(metaclass=HandlerMeta):
    pass
