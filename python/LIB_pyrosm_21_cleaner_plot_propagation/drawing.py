...
class IncrementalDrawer:
    ...
    def filename(self) -> Path:
        return self.output_dir / "graph14-{:04d}.png".format(self.file_counter)
...
class DijkstraDrawer(IncrementalDrawer):
    ...
    def on_node_settled(self, dijkstra, currently_settled_node):
        ...
        if len(self.edges_to_draw) > 10:
            hull_to_draw = compute_hull(dijkstra.G, dijkstra.settled_nodes)
            text_to_draw = f"nb settled nodes = {len(dijkstra.settled_nodes)}"
            subgraph = self.get_subgraph_to_draw(dijkstra.G)
            self.add_subgraph_and_save(subgraph, hull_to_draw, text_to_draw)
            self.begin_new_repetition()

        # DEBUG : exits after few drawings...
        if self.file_counter > 9:
            sys.exit(0)
    ...
    def add_subgraph_and_save(self, subgraph, hull_to_draw, text_to_draw):
        """
